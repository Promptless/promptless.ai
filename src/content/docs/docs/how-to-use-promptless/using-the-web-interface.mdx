...
The sidebar gives you quick access to all areas of the Promptless dashboard. The **Suggestions** entry expands to show each doc collection with a badge counting open suggestions (Ready or PR Open status). Click a collection to jump to the Suggestions page filtered to that collection. Your current status filter stays applied.
