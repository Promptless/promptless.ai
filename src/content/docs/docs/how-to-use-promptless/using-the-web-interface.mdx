...
The sidebar gives you quick access to all areas of the Promptless dashboard. The **Suggestions** entry expands to show each doc collection with a badge counting open suggestions (Ready or PR Open status). Click a collection to jump to the Suggestions page filtered to that collection. Click the **Suggestions** header to clear the filter and show all suggestions—your other filters (status, sort, assignee) stay applied.

All active doc collections appear in the list, including those with zero open suggestions. This gives you a quick view of pending documentation work across your organization without opening each collection separately.
