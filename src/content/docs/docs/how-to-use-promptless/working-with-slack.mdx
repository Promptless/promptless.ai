...
```txt wordWrap
@Promptless we just redesigned the settings UI, can you update the relevant docs with fresh screenshots?
```

```txt wordWrap
@Promptless the screenshots in our getting started guide look outdated—can you refresh them?
```

```txt wordWrap
@Promptless we shipped a new onboarding flow, please capture it for the docs
```
