...
<Tip title="Editing Suggestions in Slack">
  Promptless will reply in the thread once it's done drafting updates. From there, you can add follow-on instructions in the same thread to tell Promptless to edit the suggestion. Promptless automatically listens for replies in threads where it's participating, so you don't need to @mention it again. Start a message with `aside` or `/aside` to skip triggering Promptless for internal discussion.
</Tip>
