...
After completing the wizard, Promptless generates a `promptless.yaml` configuration from the integrations you connected. You can review and edit it anytime on the [Configuration page](/docs/configuring-promptless/configuration-reference).
...
2. **Configures triggers and context sources.** Promptless generates `promptless.yaml` entries based on the integrations you connected. Code-change integrations like GitHub, GitLab, or Bitbucket get broad triggers with `repos: all`, so Promptless listens for PRs and commits across all your repositories. Context integrations like Jira, Confluence, Linear, and Notion get unscoped entries, giving the agent access to all connected data.
