...
2. **Configures triggers and context sources.** Promptless generates `promptless.yaml` entries based on the integrations you connected. Code-change integrations like GitHub, GitLab, or Bitbucket get a broad pull request trigger with `repos: all`, so Promptless listens for pull requests across all your repositories. Promptless doesn't seed commit triggers automatically—you can add one anytime from the [Configuration page](https://app.gopromptless.ai/configuration). Context integrations like Jira, Confluence, Linear, and Notion get unscoped entries, giving the agent access to all connected data.
