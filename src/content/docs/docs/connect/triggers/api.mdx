---
title: API triggers
slug: docs/connect/triggers/api
type: reference
tags:
  - connect
  - triggers
...
<Aside type="tip">
  For interactive editor use without managing an API key, connect over MCP instead. Choose by caller: an API key suits server-to-server and CI callers, while [MCP triggers](/docs/connect/triggers/mcp) suit your editor with no key to create or paste.
</Aside>
...
<Aside type="note">
  API triggers use the same processing pipeline as other trigger types. The suggestions appear in your dashboard and follow your configured [publishing policies](/docs/reference/configuration-reference#policies).
</Aside>
