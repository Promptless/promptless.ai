...
Re-reviews an open PR each time new commits are pushed to it (available for GitHub PR triggers only). This turns Promptless into a review bot for documentation PRs, keeping its suggestions current as the PR evolves. Use it when you want an open documentation PR re-reviewed as it changes rather than analyzed once—the single-shot cadence of Opened, First approval, and Merge.
...
Because it re-reviews on *every* push, a PR that lands many incremental commits produces a matching number of re-review passes—more activity than the single-shot modes above. That volume is why the mode is gated behind explicit configuration. Scope it tightly to keep the noise manageable: name an explicit `repos` list and narrow it further with `trigger_directories`.
