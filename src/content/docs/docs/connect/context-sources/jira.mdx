...
- **Automatic Issue Retrieval**: When a GitHub PR or other trigger mentions a Jira ticket (like "PROJ-123"), Promptless automatically retrieves that issue for additional context. It reads the summary and description; the status, type, priority, and resolution; and the labels, components, and affected and fix versions. Promptless also reads the ticket's parent and subtasks, linked issues, and comments, so the agent sees how the issue fits into the surrounding work. If your organization tracks documentation-relevant details in custom Jira fields—such as a "Documentation" or "Public Description" field—Promptless reads those too, automatically and with no extra configuration. As with every field, Promptless reads only what the connected Jira account can already see, so you control its access through that account's Jira permissions and the projects you configure below.
