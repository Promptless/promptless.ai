...
When a doc collection is backed by a [GitHub (read-only)](/docs/reference/integrations/github-read-only) integration, Promptless publishes through a pull request from a fork into your upstream base branch (public repositories only; private repos need the [standard GitHub App](/docs/reference/integrations/github)). The fork is set up when you configure the doc collection. See the [GitHub (read-only) integration](/docs/reference/integrations/github-read-only) for the canonical reference on how this works.
