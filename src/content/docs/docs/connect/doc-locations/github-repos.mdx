...
When a doc collection is set up for a public repository and backed by a [GitHub (read-only)](/docs/reference/integrations/github-read-only) integration, Promptless publishes through a pull request from a fork into your upstream base branch. (This doesn't work for private repos, which need the [standard GitHub App](/docs/reference/integrations/github).) Promptless creates the fork itself — the first time it needs to work in the repository, not when you configure the doc collection. See the [GitHub (read-only) integration](/docs/reference/integrations/github-read-only) for how this works.
