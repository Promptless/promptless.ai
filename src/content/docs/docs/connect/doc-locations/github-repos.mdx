...
slug: docs/connect/doc-locations/github-repos
type: reference
tags:
  - connect
  - doc-locations
...
<Aside type="note">
  **Prerequisite**: Before creating a GitHub-based doc collection, you must first [connect GitHub](/docs/reference/integrations/github) on the integrations page. Without an active GitHub connection, you won't see any repositories in the dropdown when setting up your doc collection.
</Aside>
...
When a doc collection is set up for a public repository and backed by a [GitHub (read-only)](/docs/reference/integrations/github-read-only) integration, Promptless publishes through a pull request from a fork into your upstream base branch. (This doesn't work for private repos, which need the [standard GitHub App](/docs/reference/integrations/github).) Promptless creates the fork itself. See the [GitHub (read-only) integration](/docs/reference/integrations/github-read-only) for how this works.
...
<details>
  <summary>Promptless says it can't access the repository, or that repository access must be restored</summary>

  This happens when the connected Promptless app's access to the repository is removed after setup—for example, the Promptless GitHub App is uninstalled or suspended, the repository is deleted, or the repository is removed from the app's access list. The doc collection stays editable and removable in your configuration, but Promptless can't use it to generate documentation until access is restored.

  To fix this, confirm the repository still exists, then restore Promptless's access to it through the connected [GitHub Integration](/docs/reference/integrations/github). For public, read-only collections (`github_oss`), grant the repository to the [GitHub (read-only)](/docs/reference/integrations/github-read-only) app instead.

  When adding a collection, make sure the Promptless GitHub App can already access the repository; if it can't, grant access to that repository first (see the steps in the previous question).
</details>
