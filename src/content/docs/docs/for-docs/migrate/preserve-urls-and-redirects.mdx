...
slug: docs/for-docs/migrate/preserve-urls-and-redirects
type: guide
tags:
  - migrate
  - guide
...
| Old path                  | New path                               |
| ------------------------- | -------------------------------------- |
| `/kb/getting-started`     | `/docs/for-docs/start-here/quickstart` |
| `/reference/api-overview` | `/docs/for-docs/reference/api`         |
...
How you apply the map depends on the target platform, but the mechanism is always the same. You add a permanent (HTTP 301) redirect from each old path to its new one.
...
<Steps>
  ...
  2. **Add an entry for every changed URL.** Translate each row of your URL map into a redirect rule from the old path to the new path. Mark it permanent (301) so search engines transfer ranking to the new URL.

  3. **Handle whole-section moves.** When an entire section moves under a new prefix, a single wildcard or pattern redirect often covers all its pages. That saves you one rule per page. This requires a platform that supports pattern-based rules.
</Steps>
...
The same URL-mapping and redirect steps apply when you restructure an existing docs set rather than move platforms. See [Audit and keep your docs healthy](/docs/for-docs/audit/keep-docs-healthy) for how a restructure fits with the rest of the audit-and-health jobs.
