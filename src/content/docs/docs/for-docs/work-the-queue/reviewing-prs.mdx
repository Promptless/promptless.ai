...
**Listen to all comments** (default for new organizations): Promptless processes human comments on documentation PRs automatically. To have Promptless ignore a specific comment, start it with `aside` or `/aside`.
...
Whenever Promptless processes a comment, it checks who the comment addresses before replying. Promptless stays silent only when a comment does two things at once. First, it assigns its question or request exclusively to another named participant. Second, it asks nothing of Promptless. In that case Promptless posts no reply and makes no changes, even on a closed or merged suggestion. A comment that doesn't clearly single out someone else still gets a reply. So does a comment that still asks something of Promptless while naming another participant.

In **Require @promptless mentions** mode, a comment must mention @promptless before Promptless looks at it at all. This intake requirement is separate from the recipient check. Both mechanisms apply in this mode: a comment that mentions @promptless still goes through the recipient check.
...
A direct `@Promptless` mention works even when the comment comes from an automated account rather than a person. For example, a CI or review GitHub Action may post a comment that tags `@Promptless` on a documentation PR after a check runs. Promptless treats it as a follow-on request and acts on it, just as it does with a comment from a teammate. Bot comments that don't mention Promptless are ignored.
