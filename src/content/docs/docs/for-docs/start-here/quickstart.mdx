...
<Aside type="note">
  Connecting a code provider (GitHub, GitLab, Bitbucket) at the **Connect triggers** step is skippable—if you skip it now, you can add that provider's triggers later once you connect it, since adding or changing a provider's triggers requires that provider connected. See [Connect a provider before you add its triggers](/docs/for-docs/connect/triggers#connect-a-provider-before-you-add-its-triggers).
</Aside>

<Aside type="note">
  If you start connecting Slack during onboarding but the install doesn't finish—commonly while you wait for an admin to approve the app—the wizard shows the Slack connection as incomplete. You can select **Complete connection** once an admin approves, or **Start over**. See [Slack connection states](/docs/for-docs/reference/integrations/slack#slack-connection-states) for the full behavior.
</Aside>
