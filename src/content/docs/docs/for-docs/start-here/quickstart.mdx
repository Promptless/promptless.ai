---
title: Quickstart
description: Set up Promptless and connect your first integrations so it starts drafting documentation updates for your team.
slug: docs/for-docs/start-here/quickstart
type: guide
tags:
  - start-here
  - guide
...
id: 'connect-chat',
label: 'Connect chat',
description: 'Connect Slack or Microsoft Teams. Used for notifications and conversation-based triggers. Skippable.',
},
{
id: 'connect-docs',
label: 'Connect docs',
description: 'Install the Promptless GitHub App and pick the repos where your documentation lives. This is the only required step. Selecting a repo creates your first doc collection automatically. You can add more doc collections at any time.',
},
{
id: 'connect-triggers',
label: 'Connect triggers',
description: 'Connect source code providers (GitHub, GitLab). Changes there should trigger documentation updates. Skippable.',
},
{
id: 'connect-context',
...
<Aside type="note">
  You can skip connecting a code provider (GitHub, GitLab, Bitbucket) at the **Connect triggers** step. If you skip it now, you can add that provider's triggers later once you connect it. Adding or changing a provider's triggers requires that provider connected. See [Connect a provider before you add its triggers](/docs/for-docs/connect/triggers#connect-a-provider-before-you-add-its-triggers).
</Aside>

<Aside type="note">
  If you start connecting Slack during onboarding and the install doesn't finish, the wizard shows the Slack connection as incomplete. This commonly happens while you wait for an admin to approve the app. You can select **Complete connection** once an admin approves, or **Start over**. See [Slack connection states](/docs/for-docs/reference/integrations/slack#slack-connection-states) for the full behavior.
</Aside>

After completing the wizard, Promptless generates a `promptless.yaml` configuration from the integrations you connected. You can review and edit it anytime on the [Configuration page](/docs/for-docs/reference/configuration-reference).
...
**Triggers** - Git providers (GitHub, GitLab) as sources for pull request triggers.
...
2. **Configures triggers and context sources.** Promptless generates `promptless.yaml` entries from the integrations you connected. Code-change integrations like GitHub or GitLab get a broad pull request trigger with `repos: all`, so Promptless listens for pull requests across all your repositories. Promptless doesn't seed commit triggers automatically. You can add one anytime from the [Configuration page](https://app.gopromptless.ai/configuration). Context integrations like Jira, Confluence, Linear, Notion, and Google Drive get unscoped entries, giving the agent access to all connected data.
...
Your configuration is stored in a `promptless.yaml` file that you can view and edit anytime in the [Configuration page](https://app.gopromptless.ai/configuration). See the [Configuration Reference](/docs/for-docs/reference/configuration-reference) for the complete schema and how to narrow triggers to specific repositories.
