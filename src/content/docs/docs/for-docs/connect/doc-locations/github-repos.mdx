...
Quality problems get resolved in the background while you focus on content. With the Promptless GitHub App, this works with your existing GitHub workflow and needs no additional setup. If you connect through a custom [GitHub Enterprise](/docs/for-docs/reference/integrations/github-enterprise) app, grant it the **Checks** read permission and subscribe it to the **Check suite** webhook event. Promptless learns about CI failures only through those check suite events.
