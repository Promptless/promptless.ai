...
Reviewers without access to a private Slack conversation see Slack's "There's been a glitch" error when clicking the trigger link. To prevent confusion, private message links are clearly labeled as such ("Private Slack DM" or "Private Slack group message"). Anyone with dashboard access can read the full message on the [Triggers page](/docs/for-docs/work-the-queue/web-interface#triggers-page), regardless of their Slack permissions.
