---
title: Jira
description: Connect Jira as a read-only context source so Promptless reads the issues behind a change for requirements and business context.
...
Jira integration provides **read-only** access to your project management and issue tracking data for documentation automation. When Jira is configured as a context source, Promptless can search for related issues to understand requirements and business context. It never modifies your Jira issues.
...
- **Automatic Issue Retrieval**: When a GitHub PR or other trigger references a Jira issue (like `PROJ-123`), Promptless reads that issue for context. This happens as soon as Jira is connected. It never modifies issues, and reads only what the connected Jira account can see within the projects you configure under `project_keys` (all projects by default). Each retrieval covers:
...
- **Business Context**: Jira issues provide requirements, business logic, and feature context that enhance documentation accuracy

## Example

Jira tracks work from idea to launch, and each issue records the requirements and acceptance criteria a team agreed on before any code was written. That makes a referenced issue a reliable source for what a feature is supposed to do.
