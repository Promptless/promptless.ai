...
- **Automatic Issue Retrieval**: When a GitHub PR or other trigger mentions a Jira ticket (like "PROJ-123"), Promptless automatically retrieves that issue for additional context—no setup required beyond connecting Jira. It reads a broad set of standard fields:

  - the summary and description;
  - the status, type, priority, and resolution;
  - the labels, components, and affected and fix versions;
  - the parent and subtasks, linked issues, and comments, so the agent sees how the issue fits into the surrounding work.

  Promptless can also read your organization's custom Jira fields when they hold documentation-relevant context—for example, a "Documentation" or "Public Description" field. It reads only what the connected Jira account can already see, so that account's permissions and the projects you configure below govern what it retrieves.
