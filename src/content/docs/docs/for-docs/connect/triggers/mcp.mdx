...
Use `answer_task_question` to answer a clarifying question a task marked, whether it's still working or has finished. Use `send_task_message` to add an instruction to a task that's still running. For the full walkthrough, see [Answer a clarifying question](#answer-a-clarifying-question).

Answers the one clarifying question a task marked, whether the task is still working or has finished, so Promptless continues the work.

| Argument              | Required | Description                                                                                                                                      |
| --------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `trigger_event_id`    | Yes      | The task whose question you're answering (UUID).                                                                                                 |
...
It returns the `question` text, an `outcome`, a `trigger_event_id` to follow, an optional `next_call`, and a `message`. The `outcome` is one of five values:

- `submitted`. The answer is accepted; follow `next_call`. On a task that's still working, the answer commits to that same task and clears the pending question. Then `next_call` and `trigger_event_id` point back at that task, and Promptless keeps working it. On a finished task, the answer starts a new continuation task instead. That continuation carries the original request, context, clarification history, and links to any results, and `next_call` and `trigger_event_id` point at it.
- `already_answered`. The question already had an answer committed, by an earlier call or an overlapping native form. Retrying with the same answer returns that committed answer instead of erroring. If a different answer won first, yours isn't submitted. Follow the task the returned `trigger_event_id` names. That's the same task while it's still working, or the continuation once it has finished.
...
A task can mark one clarifying question in your editor, while it's still working or when it finishes. This applies to a task you started over MCP. What happens when you answer depends on the task's state at that moment. If the task is still working, your answer commits to that same task. The pending question clears, Promptless keeps working it, and it can mark a further question the same way. If the task had already finished, your answer starts a new task instead. That task carries the original request, the original context, the clarification history, and links to any results already produced. Either way, you pick up directly from the clarification.
...
Your editor follows the accepted answer via its `next_call`. That points at the same task if it was still working, or at the new continuation if it had finished. The first committed answer wins. Answering the same question again returns `already_answered`, and a matching answer comes back unchanged. If a different answer won first, yours isn't submitted, and you follow the task the response names to pick up the committed one. If you decline or cancel the question, no work continues and following stops. The original task and anything it already produced, such as suggestions, remain. To pursue the change, start a new task with `submit_documentation_task`.
...
A task can mark one clarifying question, while it's working or when it finishes. Answer it in plain language: "Answer Promptless task `<id>`'s clarifying question: yes, cover the deprecation notice." Your editor calls `answer_task_question`, and Promptless picks up your answer. It picks up on the same task if it's still working, or in a new task if it had finished. Your editor follows that task via its `next_call`.
...
- **`answer_task_question` returns `already_answered`**: the question already had an answer committed. Follow the task the returned `trigger_event_id` names. That's the same task if it's still working, or the continuation already created if it had finished. If a different answer won first, yours wasn't submitted; following that task shows the committed answer.
