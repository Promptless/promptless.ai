...
Through your editor, these 13 tools let you check on your collections, tasks, and suggestions, or queue and change your documentation work. Six tools are read-only: `list_doc_collections`, `get_task_status`, `list_recent_tasks`, `search_suggestions`, `list_doc_update_findings`, and `wait_for_task_update`. Your editor can auto-approve them, so routine status checks stop prompting you each time. Seven tools queue or change work: `submit_documentation_task`, `send_task_message`, `answer_task_question`, `request_changes`, `update_suggestion`, `draft_doc_update_finding`, and `close_suggestion`. These always ask before they run.

<Aside type="note">
  If you already have an MCP connection, refresh or reconnect the `promptless` server in your editor. That loads the `send_task_message`, `answer_task_question`, `wait_for_task_update`, `list_doc_update_findings`, and `draft_doc_update_finding` tools and the updated tool descriptions. `submit_documentation_task` and `request_changes` now return a `next_call` for following the task. In Claude Code, run `/mcp` and pick **promptless** again, or start a new conversation. In Cursor, remove the `promptless` server and add it again.
</Aside>
...
Use `answer_task_question` to answer a clarifying question a finished task marked. Use `send_task_message` to add an instruction to a task that's still running. For the full walkthrough, see [Answer a clarifying question](#answer-a-clarifying-question).
...
### `list_doc_update_findings`

Lists your recorded doc update findings, newest first. Use it to see what Promptless planned but hasn't drafted, and to get a `finding_id` to pass to `draft_doc_update_finding`. It's read-only, and your editor can auto-approve it.

| Argument            | Required | Description                                                                       |
| ------------------- | -------- | --------------------------------------------------------------------------------- |
| `doc_collection_id` | No       | Restrict results to one collection. Omit to list findings across all collections. |
| `status`            | No       | Restrict results to one of `open`, `drafting`, `drafted`, or `dismissed`.         |
| `limit`             | No       | Caps how many findings come back, from 1 to 100. Defaults to 25.                  |

The response returns the matching findings in `doc_update_findings`, the `limit` applied, and a `truncated` flag that's true when more findings match than were returned.

### `draft_doc_update_finding`

Drafts a recorded doc update finding, turning the plan into an ordinary suggestion. Promptless resumes the original trigger with the finding's research attached and starts authoring. Only an `open` finding can be drafted.

| Argument       | Required | Description                                             |
| -------------- | -------- | ------------------------------------------------------- |
| `finding_id`   | Yes      | The finding to draft (from `list_doc_update_findings`). |
| `instructions` | No       | Extra guidance for the resulting suggestion.            |

It returns the `trigger_event_id`, the `finding_id`, the finding's new `status`, a `message`, and a `next_call`. Follow `next_call` with `wait_for_task_update` to reach the suggestion it produces. Like the other work-changing tools, it prompts before it runs and isn't auto-approved.

There's no MCP tool to dismiss a finding. Dismiss one from the Doc Plans page, or call [`POST /v1/doc-update-findings/{id}/dismiss`](/docs/for-docs/connect/triggers/api#dismiss-a-doc-update-finding).
...
- **Drafting a finding that isn't open**: `draft_doc_update_finding` reports that the finding isn't open, naming its current status. Call `list_doc_update_findings` with `status: open` to find a finding you can draft.
- **An unrecognized finding id**: `draft_doc_update_finding` reports that no finding matches the given ID. Call `list_doc_update_findings` for a valid id.
