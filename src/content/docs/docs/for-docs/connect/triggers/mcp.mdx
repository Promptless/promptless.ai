...
Connect an MCP-capable editor (Claude Code, Cursor, or another) to Promptless. Then, from the editor you already have open, start a documentation task and get its outcome and docs pull request link back. You can also ask Promptless to revise a suggestion it already made. You can send a follow-up instruction into a running task and read that task's conversation. You can set a suggestion's labels, assignees, and title, open its pull request, or close it yourself. There's no API key to create, store, or rotate: you authorize once in a browser instead.
...
Through your editor, these nine tools let you check on your collections, tasks, and suggestions, or queue and change your documentation work. Four tools are read-only: `list_doc_collections`, `get_task_status`, `list_recent_tasks`, and `search_suggestions`. Your editor can auto-approve them, so routine status checks stop prompting you each time. Five tools queue or change work: `submit_documentation_task`, `send_task_message`, `request_changes`, `update_suggestion`, and `close_suggestion`. These always ask before they run.

<Aside type="note">
  If you already have an MCP connection, refresh or reconnect the `promptless` server in your editor. That loads the `send_task_message` tool and its updated tool description. In Claude Code, run `/mcp` and pick **promptless** again, or start a new conversation. In Cursor, remove the `promptless` server and add it again.
</Aside>
...
Each message carries a stable, immutable `id`, a `sequence` number, an `author` object, a `created_at`, and the `message` text. A caller that polls repeatedly uses the `id` to dedupe, relaying each new message only once. Your editor tracks these `id`s for you, so it doesn't relay the same message twice. The `author` object's `type` is `promptless` or `customer`, and its `name` is the customer's email when available, otherwise null.

A `status_guidance` field is a short text string carrying server-generated advice about polling the task and interpreting its outcome. That advice is distinct from the conversation itself, which `messages` carries.
...
Sends an additional instruction into a task that's already running. The instruction folds into that running task's work, and no new task starts. A task stops accepting messages once it finishes, including a task that finished by asking a clarifying question or reporting a blocker.
...
Skills are purely additive. A client that doesn't support the extension, or ignores it, sees exactly the same nine tools; the guidance costs nothing when it goes unused.
...
- **Sending to a finished task**: the tool rejects it with `task_finished`. A task that finished by reporting a blocker or a clarification request has still finished, so it also rejects messages. Start a new task with `submit_documentation_task` to continue.
