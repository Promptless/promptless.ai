...
Sends an additional instruction into a task that's already running. The instruction folds into that running task's work, and no new task starts. A task stops accepting messages once it finishes, including one that finished by asking a clarifying question or reporting a blocker. If the finished task marked a clarifying question, answer it with [`answer_task_question`](#answer-a-clarifying-question). That call continues the work in a new task and leaves this one finished.
...
A task can finish by asking one. Answer it in plain language: "Answer Promptless task `<id>`'s clarifying question: yes, cover the deprecation notice." Your editor calls `answer_task_question`, and Promptless continues the work in a new task your editor follows via its `next_call`.
