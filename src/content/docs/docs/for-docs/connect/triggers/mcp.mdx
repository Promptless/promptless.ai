...
It also returns the suggestions the task produced. Each comes with its docs pull request link and review status, using the same `draft`, `open`, `merged`, or `closed` values `search_suggestions` uses. The response says whether the task has finished. An `outcome` field names how a finished task ended, as one of `suggestions_created`, `no_change_needed`, `needs_input`, or `failed`. Branch on `outcome` to route on the result. It is `null` until the task finishes, and for a task from a source other than the API or MCP. A `resolution` note explains the same verdict in prose.
