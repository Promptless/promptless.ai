...
- **Sending to a task whose source has no conversation**: the tool rejects it with `unsupported_task_source`. Task conversations cover API and MCP submissions, tasks created by calling `request_changes` over MCP, and New Tasks submitted from the dashboard. Read the task's status instead, or start a task that carries a conversation to converse.
