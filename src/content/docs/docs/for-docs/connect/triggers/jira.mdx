...
<Aside type="note">
  For a custom integration that lets Jira Automation's own condition builder decide when to call Promptless, see [Trigger Promptless from Jira Automation](/docs/for-docs/connect/triggers/jira-automation-api). The built-in paths on this page remain the recommended way to trigger from Jira.
</Aside>
