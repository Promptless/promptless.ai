...
The fourth path is a configured Jira trigger that activates when an issue is created, when a label is added, or when an issue changes status. It is unaffected by the service account and starts sessions whether or not one is attached. Like the built-in paths, Promptless comments on the ticket and flags it for follow-up if it goes unanswered.
...
Attaching a dedicated Atlassian service account gives Promptless its own bot identity: it posts as that bot, and the mention and assignment paths start working. Either way, Promptless ignores its own activity—self-echo suppression keys on the connecting account when no service account is attached, and on the service account when one is.
