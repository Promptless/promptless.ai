...
<Aside type="tip">
  Replay is also available for [GitLab merge requests](/docs/for-docs/connect/triggers/gitlab-merge-requests#replay-recent-merge-requests) and [Bitbucket pull requests](/docs/for-docs/reference/integrations/bitbucket#replay-recent-prs). When you enable multiple code review triggers during onboarding, the replay preview groups results by provider.
</Aside>
...
5. Add each folder path — one chip per path, separated by a comma or Enter.
