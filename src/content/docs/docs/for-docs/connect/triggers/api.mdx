...
Read your organization's finished tasks, newest completion first. Poll this feed to learn that a task ended, including a task that deliberately produced no documentation change. For the full schema, see the [List finished tasks reference](/docs/for-docs/api/operations/listfinishedtasks/).

Only finished tasks appear in this feed. A task still running has no outcome yet, so follow one in flight with [`GET /v1/triggers/{trigger_event_id}`](#read-task-status). The feed also omits Promptless's internal re-runs, so each finished task appears once.
...
| Field              | Type           | Description                                                                                  |
| ------------------ | -------------- | -------------------------------------------------------------------------------------------- |
| `trigger_event_id` | string (UUID)  | The task's id, the same id `POST /v1/triggers` returned. Deduplicate a feed on this value.   |
| `source`           | string         | The task's origin, such as `api`.                                                            |
| `request`          | string \| null | A one-line summary of the submitted request.                                                 |
| `outcome`          | string \| null | How the task ended, as one of the fixed values below. Branch on this field.                  |
| `resolution`       | string \| null | A human-readable note that explains the same verdict in prose, written for a person to read. |
| `submitted_at`     | string         | ISO 8601 time the task was submitted.                                                        |
| `finished_at`      | string         | ISO 8601 time the task finished.                                                             |

#### Outcomes

`outcome` is one of a fixed set of values an automation can filter or branch on:
...
`outcome` is `null` in two cases: a task that finished before Promptless recorded outcomes, and a task from a source other than the API or MCP. Those sources deliver the outcome as a reply in the source thread. Treat this list as the contract and handle an unrecognized value gracefully, because Promptless can add an outcome.
...
- Since `since` is inclusive, a task whose `finished_at` equals `since` comes back again. Deduplicate by `trigger_event_id`.
...
| `request`          | string \| null | A one-line summary of the submitted request.                                                                                                                                                                                    |
| `resolution`       | string \| null | A human-readable note that explains the outcome in prose, written for a person to read. Read it for context after `finished` is `true`.                                                                                         |
| `outcome`          | string \| null | How the task ended, as one of a fixed set of values an automation can branch on (see [Finished tasks](#finished-tasks) for the values). `null` until the task finishes, and for a task from a source other than the API or MCP. |
