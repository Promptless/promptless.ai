...
<Aside type="tip">
  When rotating keys, update your integrations before generating a new key. The previous key stops working immediately when you create a new one. Because your organization has a single active key shared by every API caller, rotating it breaks all of them at once with no overlap window—coordinate the change so every consumer is updated together.
</Aside>
