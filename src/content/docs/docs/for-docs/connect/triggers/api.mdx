...
1. Go to [**Settings > API Access**](https://app.gopromptless.ai/settings/api).
...
Lists your organization's finished tasks, newest completion first, including tasks that made no documentation change. For the full schema, see the [List finished tasks reference](/docs/for-docs/api/operations/listfinishedtasks/).

Only finished tasks appear in this feed. A task still running has no outcome yet, so follow one in flight with [`GET /v1/triggers/{trigger_event_id}`](#read-task-status). The feed omits Promptless's internal re-runs. If Promptless reopens a finished task to rework it, the task appears again when it finishes, with a new `finished_at` and possibly a different `outcome`.
...
| `outcome`          | string \| null | How the task ended. See [Outcomes](#outcomes).           |
...
This feed omits the suggestions a task produced. Each suggestion in the [Suggestions](#suggestions) feed carries `trigger_event_id`, so join the two feeds on that field.
...
Poll this feed the way you poll the [Suggestions](#suggestions) feed, ordered by when each task completed:
...
| `outcome`          | string \| null | How the task ended. See [Outcomes](#outcomes) for the values. `null` until the task finishes, and for tasks from sources other than the API or MCP.                          |
...
API-triggered events appear in your dashboard with an **API Task** pill. A task that reaches the API through a Promptless-built client carries that client's own pill. A task submitted through the Promptless Zapier app shows a **Zapier Task** pill. A task submitted from your editor over [MCP](/docs/for-docs/connect/triggers/mcp) shows an **MCP Task** pill.
