...
description: Trigger documentation updates from Microsoft Teams messages and mentions, so questions raised in chat become documentation suggestions.
slug: docs/for-docs/connect/triggers/microsoft-teams-messages
type: reference
tags:
  - connect
  - triggers
...
<Aside type="tip" title="Opting Out of Individual Messages">
  Promptless tries to reason about whether each message in a thread is actionable, so not every reply will trigger a documentation update. However, if you want to explicitly tell Promptless to ignore a message, start it with `aside` or `/aside`:
...
<Aside type="note">
  Adding, renaming, or modifying a Microsoft Teams trigger requires the Microsoft Teams integration to be connected first. See [Connect a provider before you add its triggers](/docs/for-docs/connect/triggers#connect-a-provider-before-you-add-its-triggers).
</Aside>
...
Publishing behavior and notifications are controlled by your [policies configuration](/docs/for-docs/reference/configuration-reference#policies).
...
To connect Microsoft Teams to Promptless, see the [Microsoft Teams Integration](/docs/for-docs/reference/integrations/microsoft-teams) setup guide.
