...
<img src="https://promptless-customer-doc-assets.s3.us-east-2.amazonaws.com/docs-images/org_3DaVKUzHgjyjaODgCPaDmjXezqp/new-task-composer-deep-analysis-fullpage-bc738009.png" alt="The full New Task page with the New Task tab highlighted in the left icon sidebar. The composer shows the Doc Collection chip set to Let Promptless decide, the Mode chip set to Deep analysis, and the three-segment Notify toggle with its Slack segment selected. Below an empty instructions field with a paperclip and submit arrow sit the Auto-create PRs chip and a chip reading Suggestions and blockers go to # doc-notifications. The Use a template rail shows the template cards." />
...
<Steps>
  ...
  7. **Choose delivery settings.** The **Notify** toggle picks where this task's suggestion notifications and blockers go in Slack. The chip below the composer names the channel: **Suggestions and blockers go to** `#channel`. The toggle preselects your organization's configured Slack channel. Promptless resolves it from your `promptless.yaml` policies: the notification channel, or the escalation channel when that's the only one set. The preselected channel follows the **Doc Collection** you choose, so it matches where this run posts. It stays on **No notification** when your organization has no Slack channel configured. You can pick a different Slack channel. Choose **No notification** to keep the task out of Slack entirely, blockers included. Turn on **Auto-create PRs** to open PRs automatically for this task's suggestions and skip the manual review step for each one.

  8. **Submit.** Click the submit arrow at the bottom right of the instructions field. Pressing Enter adds a newline instead of submitting. Promptless opens the task's conversation page, where it acknowledges the request, posts progress, and reports the outcome. You can reply there while the run works. See [Task conversations](/docs/for-docs/work-the-queue/web-interface#task-conversations).
</Steps>
...
Promptless posts progress in the task's conversation as the run works. If you picked or kept a Slack channel, Promptless posts there when the suggestions are ready for review. It also posts there when it needs your input to continue.
