...
<Aside type="tip" title="Before You Begin">
  ...
  - **For Microsoft Teams, install the Promptless Teams app** with permission to read channel messages, plus admin consent, so it can read channel conversations without an @mention. Teams support is in beta—contact [help@gopromptless.ai](mailto\:help@gopromptless.ai) to enable it for your organization. See the [Microsoft Teams integration](/docs/for-docs/reference/integrations/microsoft-teams) setup guide for installation.
