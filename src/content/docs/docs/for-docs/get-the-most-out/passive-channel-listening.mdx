...
<Aside type="tip" title="Before You Begin">
  - **When you configure a public Slack channel for passive listening, Promptless joins that channel**, so you don't need to invite it. Promptless can't join a private channel or a channel shared with another workspace through Slack Connect on its own. For those, [add Promptless in Slack](/docs/for-docs/reference/integrations/slack#add-promptless-to-private-channels) yourself so it can read messages there.
...
For Slack trigger details, see [Slack messages](/docs/for-docs/connect/triggers/slack-messages). For other ways to trigger Promptless from Teams, see [Microsoft Teams](/docs/for-docs/work-the-queue/reviewing-from-slack-and-teams#microsoft-teams). For the full set of match fields, see the [Configuration Reference](/docs/for-docs/reference/configuration-reference#triggers).
