...
<Aside type="note">
  Promptless Capture is available to every organization—there's nothing to turn on. The only prerequisite is credentials, and you need them only when the product you want to capture is behind a login. A public, unauthenticated interface needs none. See [Set up credentials](#set-up-credentials).
</Aside>

<Aside type="note">
  Promptless Capture generates screenshots and keeps them current; to review the screenshots already attached to your open suggestions, use the [Screenshot Updates](/docs/for-docs/work-the-queue/web-interface#screenshot-updates) tab.
</Aside>
