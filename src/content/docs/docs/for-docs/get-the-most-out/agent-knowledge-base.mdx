...
## Access from your own tooling

Read and write these files programmatically with your Promptless API key. Clone the Agent Knowledge Base over Git to read the whole tree with its history. Create or replace one file at a time over the API, so your own tools and automation work from the same context Promptless reads. See [Agent Knowledge Base access](/docs/for-docs/connect/triggers/api#agent-knowledge-base-access) on the API triggers page.
