...
<Steps>
  ...
  4. **Enter the group path and paste the token.** In the **Group path** field, enter the group's full path, including parent groups if the group is nested. Paste the token into the **Access token** field, then click **Connect**.
