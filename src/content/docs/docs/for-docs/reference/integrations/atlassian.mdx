...
<Aside type="note">
  Confluence scopes include both classic and granular variants for compatibility with Atlassian's API v2. Promptless writes to Jira only when a [Jira trigger](/docs/for-docs/connect/triggers/jira) runs—posting the reply comment and attaching the documentation PR as a remote link—never proactively, and never to Confluence. This write happens whether or not a [service account](#atlassian-service-account) is attached; the service account only changes the identity Promptless posts as (a dedicated bot account instead of the connecting user). Reading Jira and Confluence for context remains read-only.
</Aside>
