...
- **When it happens**: Never automatically, and never tied to a trigger. Contrast this with the reply Promptless posts automatically when a [Jira trigger](/docs/for-docs/connect/triggers/jira) runs.
- **Duplicate check**: Before creating an issue, Promptless checks the target project for an issue with a matching title. When it finds one, Promptless reuses that issue instead of creating a new one.
- **Custom fields**: Promptless cannot set custom fields on an issue it creates this way.
- **Attribution**: A created issue's **Reporter** is always the connecting account, the Jira user who authorized the Atlassian OAuth connection. This never changes when you attach an Atlassian service account. Replies move to the service account once one is attached, but a created issue stays with the connecting account.
- **Comments on Jira**: The only comment Promptless posts to Jira is its reply on the ticket that started a session (see the [Jira trigger](/docs/for-docs/connect/triggers/jira) page).
- **Permissions**: Issue creation uses the `write:jira-work` scope Promptless already holds. No reconnect is needed.
