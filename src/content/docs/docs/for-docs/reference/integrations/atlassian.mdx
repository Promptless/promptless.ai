...
**Identity implications.** Whichever account Promptless acts as is the identity behind assignment triggering, mention matching, and ignoring its own activity. Attaching a service account moves that identity off the connecting admin. Without one, the connecting admin is that identity—so assigning a ticket to them starts a session, and their own ticket activity is ignored.
