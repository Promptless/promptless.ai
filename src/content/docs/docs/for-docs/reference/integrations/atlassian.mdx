...
<details>
  <summary>Promptless stopped responding to Jira @mentions and ticket assignments</summary>

  The mention and assignment paths require an attached Atlassian service account; without one, those deliveries do nothing. Attach a service account (see [Atlassian service account](#atlassian-service-account)) to turn those paths back on. The built-in `promptless`-label path and configured Jira triggers still start sessions without a service account.
</details>
