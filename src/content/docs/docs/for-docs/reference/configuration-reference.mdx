...
The escalation channel is where Promptless reports configuration and operational problems, separately from notifications about suggestions being created, updated, closed, or merged. It receives two kinds of reports. One is a problem Promptless hits while working on a trigger, such as a PR that references a Jira issue Promptless can't read. The other is an alert from a background check, such as the daily [connection check](/docs/for-docs/reference/integrations#proactive-notifications) finding an integration that needs reconnecting. See [Escalation channel](/docs/for-docs/tune/notifications#escalation-channel) for the full behavior. It's a sibling of `notification` and, unlike `notification`, also accepts `email`.
...
Set any combination of the three. The same presence semantics as `notification` apply: omit to inherit; `null` for `slack_channel`/`msteams_channel` or `[]` for `email` to disable. `msteams_channel` carries the same Teams-activity prerequisite as `notification`. Escalations reach the escalation channel(s) even when suggestion notifications are off or no notification channel is set. If you omit `slack_channel` or `msteams_channel`, problems from trigger work go to the `notification` channel on that platform. Set it to `null` to send them nowhere on that platform. Background-check alerts, such as an integration needing reconnecting, go only to channels set under `escalation`.

<Aside type="caution">
  If you set only a `notification` channel, background-check alerts reach no channel. Set an `escalation` destination to receive them; see [Escalation channel](/docs/for-docs/tune/notifications#escalation-channel) for the full behavior.
</Aside>
