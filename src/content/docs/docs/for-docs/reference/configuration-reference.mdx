...
The escalation channel is where Promptless reports configuration and operational problems, separately from notifications about suggestions being created, updated, closed, or merged. It receives two kinds of problem reports. A trigger problem is one Promptless hits while working on a trigger, such as a PR that references a Jira issue Promptless can't read. A connection alert comes from the daily [connection check](/docs/for-docs/reference/integrations#proactive-notifications), such as an integration that needs reconnecting. See [Escalation channel](/docs/for-docs/tune/notifications#escalation-channel) for the full behavior. It's a sibling of `notification` and, unlike `notification`, also accepts `email`.
...
Set any combination of the three. The same presence semantics as `notification` apply: omit to inherit; `null` for `slack_channel`/`msteams_channel` or `[]` for `email` to disable. `msteams_channel` carries the same Teams-activity prerequisite as `notification`. Escalations reach the escalation channel(s) even when suggestion notifications are off or no notification channel is set. A problem report goes to a platform's `notification` channel only when it's a trigger problem and the resolved `escalation` section omits that platform's field. A `null` field sends it nowhere on that platform. Connection alerts go only to the `escalation` destinations in `policies.default`. See [When problem reports go to the notification channel](/docs/for-docs/tune/notifications#when-problem-reports-go-to-the-notification-channel) for an example.

<Aside type="caution">
  If you set only a `notification` channel, connection alerts reach no channel. Set an `escalation` destination in `policies.default` to receive them.
</Aside>

`escalation.email` routes escalations to a list of email addresses. Email is escalation-only. Suggestion notifications never arrive by email. `notification` has no email setting, so Promptless emails escalations only when you set `escalation.email`. Promptless emails only the addresses you list. Email escalation requires email delivery to be configured for your deployment. It's on by default on Promptless-hosted deployments. Self-hosted deployments must configure it.
...
<Aside type="tip">
  If you edit the configuration file directly via git push, invalid YAML keeps the last valid configuration active. The error appears in the Configuration page editor.
</Aside>
