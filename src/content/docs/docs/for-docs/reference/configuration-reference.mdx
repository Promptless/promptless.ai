...
In Form mode, a trigger type whose provider you haven't connected is disabled, with a link to connect that provider on the Integrations page. Schedule triggers need no integration and stay available.

A saved trigger row whose provider is disconnected shows as inactive: its match fields are locked, while **Remove** stays available. The row reactivates once you reconnect the provider.

A save that adds, renames, or modifies a provider-backed trigger while its integration is disconnected is rejected before anything is committed. It never reaches your `promptless.yaml` or your active configuration. Reconnect the provider, then save again. This prerequisite is covered in full under [Connect a provider before you add its triggers](/docs/for-docs/connect/triggers#connect-a-provider-before-you-add-its-triggers).
...
Before adding or changing a trigger, note that provider-backed triggers require their integration to be connected. See [Provider connection and trigger availability](#provider-connection-and-trigger-availability).
...
- A provider-backed trigger added, renamed, or modified while its required integration is disconnected (for example, a GitHub trigger while GitHub is disconnected). See [Connect a provider before you add its triggers](/docs/for-docs/connect/triggers#connect-a-provider-before-you-add-its-triggers)
