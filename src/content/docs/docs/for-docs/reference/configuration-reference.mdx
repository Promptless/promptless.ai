...
The escalation channel is where Promptless routes configuration and operational alerts, rather than suggestion announcements. This is completely different from notifications about suggestions being created, updated, closed, or merged. It covers agent-authored escalations, the ones Promptless raises while working a trigger, such as a test account missing the feature flags to screenshot a screen, or a PR that references a Jira issue Promptless can't read because the Jira integration isn't connected or the project isn't accessible. It also covers deterministic and operational alerts raised by ongoing monitoring, such as integration-health [proactive notifications](/docs/for-docs/reference/integrations#proactive-notifications) when a connected integration develops a problem. See [Escalation channel](/docs/for-docs/tune/notifications#escalation-channel) for the full behavior. It's a sibling of `notification` and, unlike `notification`, also accepts `email`.
...
Set any combination of the three; the same presence semantics as `notification` apply (omit to inherit; `null` for `slack_channel`/`msteams_channel` or `[]` for `email` to disable), and `msteams_channel` carries the same Teams-activity prerequisite as `notification`. Escalations reach the escalation channel(s) even when suggestion notifications are off or no notification channel is set. When a surface's escalation channel is unset, only agent-authored escalations fall back to that surface's notification channel, resolved per surface: omitting the escalation field lets agent-authored escalations fall back to the notification channel, while setting it to `null` sends nothing on that surface. Deterministic and operational alerts, including integration-health alerts, are never rerouted this way. They need an explicit escalation destination and stay silent until one is set.

<Aside type="caution">
  Configure only a notification channel and no escalation destination, and integration-health and operational alerts go nowhere. Set an explicit `escalation` destination to receive them; see [Escalation channel](/docs/for-docs/tune/notifications#escalation-channel) for the full behavior.
</Aside>

`escalation.email` routes escalations to a list of email addresses. Email is escalation-only. Suggestion notifications never arrive by email. The `notification` block has no email counterpart, so escalations reach email only when you set `escalation.email` explicitly. Promptless emails only the addresses you list. Email escalation requires email delivery to be configured for your deployment. It's on by default on Promptless-hosted deployments. Self-hosted deployments must configure it.
