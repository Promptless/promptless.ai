...
## Configure search and the assistant
...
To turn on the optional site assistant, set `ANTHROPIC_API_KEY` in `.env` and the assistant flag in the search options. The key comes from your Anthropic account. `STARPORT_ASSISTANT_MODEL` selects the model, and defaults to `claude-sonnet-5`.
...
Starport deploys to Vercel first, with documented paths for GitHub Pages and Cloudflare Pages. The docs MCP server is enabled by default through `MCP_ENABLED`, and you can turn it off. It needs an on-demand runtime, so set `MCP_RUNTIME` in `.env`, which defaults to `vercel`.
