...
Stating a full format like this replaces the default message format. Promptless posts exactly the fields and links you list without adding its default links on top. The example above lists no Knowledge Base link, so its notifications carry none. The default is a short prose summary followed by a dashboard review link and the docs PR or MR link. When the trigger that started the work names a merge request or ticket, the default also links back to it. It also carries one Knowledge Base link per changed file and any requested mentions. This default applies in full when the preferences file is absent or still holds only its scaffolded placeholder text. If the file exists but can't be read, Promptless holds the notification rather than sending one in the default format.
