...
<Aside type="caution">
  Deterministic and operational alerts, including integration-health alerts, need an explicit `escalation` destination and stay silent until one is set. Agent-authored escalations fall back to the `notification` channel only when the `escalation` channel is omitted, not set to `null`. See [Escalation channel](#escalation-channel).
</Aside>
...
Escalation channels notify you of configuration and operational problems. This is completely different from notifications about suggestions being created, updated, closed, or merged.

Escalations come from two places, and the difference matters for routing. Agent-authored escalations are the ones Promptless raises while working a trigger, for example:

- An integration it couldn't reach
- An expired or rejected credential it hit while researching a change
- A test account that lacks the feature flags to reach a screen Promptless needs to screenshot
- A PR that references a Jira issue Promptless can't read, because the integration isn't connected or the project isn't accessible

Deterministic and operational alerts, including integration-health [proactive notifications](/docs/for-docs/reference/integrations#proactive-notifications), are raised outside any trigger, by ongoing monitoring instead. Only agent-authored escalations fall back to notification routing; deterministic alerts never do. Without an escalation destination configured, these problems surface only on the [Integrations page](https://app.gopromptless.ai/integrations), which you check manually.
...
How each surface routes depends on whether its escalation channel is set:

- **Omitted:** only agent-authored escalations fall back to that surface's notification channel. The fallback resolves per surface, Slack and Teams independently, so an unset escalation channel on one surface doesn't change the other.
- **A channel name or ID:** overrides the fallback and receives escalations directly.
- **`null`:** disables that surface, so nothing falls back there.

Deterministic and operational alerts, including integration-health alerts, are never rerouted to a notification channel. They require an explicit escalation destination and stay silent until one is set.

<Aside type="caution">
  If you configure only a notification channel and no escalation destination, you will not receive integration-health or operational alerts. Set an explicit escalation destination (`escalation.slack_channel`, `escalation.msteams_channel`, or `escalation.email`) to receive them.
</Aside>

To start receiving integration-health and operational alerts, add an `escalation` channel under an already-populated `notification` block:
...
List one or more addresses under `escalation.email`. Email is escalation-only. Suggestion notifications never arrive by email. The `notification` block has no email counterpart, so escalations of either kind reach email only when you set `escalation.email` explicitly. Promptless emails only the addresses you list. Email escalation requires email delivery to be configured for your deployment. It's on by default on Promptless-hosted deployments. Self-hosted deployments must configure it.
