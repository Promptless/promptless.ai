...
- **Message format**: Define the fields, order, and links your notifications carry. This replaces Promptless's default message shape rather than adding to it. A stated format is exhaustive, and Promptless adds nothing it doesn't list
...
Stating a full format like this replaces the default message shape. Promptless posts exactly the fields and links you list, and doesn't add its default links on top. The example above lists no Knowledge Base link, so its notifications carry none. The default message shape is a short prose summary followed by a dashboard review link and the docs PR or MR link. It also carries one Knowledge Base link per changed file and any requested mentions. This default applies in full when the preferences file is absent or still holds only its scaffolded placeholder text. If the file exists but can't be read, Promptless holds the notification rather than sending one in the default shape.
