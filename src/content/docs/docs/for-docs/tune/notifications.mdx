...
<Aside type="caution">
  Your `notification` channel receives some problem reports, but not all of them. For example, an alert that your GitHub integration needs reconnecting goes only to an `escalation` channel. See [Escalation channel](#escalation-channel) for which reports reach which channel.
</Aside>
...
Escalation channels notify you of configuration and operational problems, separately from notifications about suggestions being created, updated, closed, or merged. Promptless sends two kinds of escalations:

- Problems Promptless hits while working on a trigger. For example, a PR references a Jira issue Promptless can't read because the Jira integration isn't connected. Or the test account lacks the feature flags to reach a screen Promptless needs to screenshot.
- Alerts from background checks, such as the daily [connection check](/docs/for-docs/reference/integrations#proactive-notifications) finding an integration that needs reconnecting.
...
Escalations reach these channels even when suggestion notifications are off or no notification channel is set. `slack_channel` routes escalations to Slack; `msteams_channel` routes them to Teams.

When you leave out an escalation channel, problems from trigger work go to the notification channel on the same platform. Background-check alerts go only to channels set under `escalation`. For example, this policy sets an escalation channel for Slack and leaves Teams out:
...
- An unreadable Jira issue during a trigger goes to `docs-escalations` in Slack. In Teams, it goes to the notification channel, `19:0a1b2c3d@thread.tacv2`.
- An alert that the GitHub integration needs reconnecting goes to `docs-escalations` only. Teams gets nothing.
- Adding `msteams_channel: null` under `escalation` keeps the unreadable Jira issue out of Teams too.

<Aside type="caution">
  If you set only a `notification` channel, background-check alerts reach no channel. A broken integration then shows up only on the [Integrations page](https://app.gopromptless.ai/integrations).

  To receive these alerts, set at least one of `slack_channel`, `msteams_channel`, or `email` under `escalation`.
</Aside>
