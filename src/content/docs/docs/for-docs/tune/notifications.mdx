...
<Aside type="caution">
  The `notification` channels cover suggestion activity only. Operational and integration-health alerts require the separate `escalation` configuration described later on this page. See [Escalation channel](#escalation-channel).
</Aside>
