...
A [policy rule](#apply-rules-per-collection) applies based only on a trigger and a doc collection, and it sets only notification, publishing, and archiving fields — and archiving responds only to rules scoped by doc collection, not by trigger. Preferences that don't fit those fields belong in the `PROMPTLESS.md` file in your [Agent Knowledge Base](/docs/get-the-most-out/agent-knowledge-base), which Promptless reads at the start of every documentation task. That file is where each team's own conventions and exceptions live — the judgment a policy rule has no field for.
