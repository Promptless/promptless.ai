...
Because directory targeting is relevance-based, your lever is `PORMPTLESS.md` in your [Agent Knowledge Base](/docs/get-the-most-out/agent-knowledge-base). Tell it which directory holds the current version and which directories hold frozen releases so it edits the right one. For example:
