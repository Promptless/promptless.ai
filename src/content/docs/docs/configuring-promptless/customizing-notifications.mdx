...
When enabled, Promptless posts a message in the original Slack thread when a suggestion reaches its final state:

- **Merged** — The documentation PR was merged and deployed
- **Closed** — The documentation PR was closed without merging
- **Rejected** — The suggestion was rejected from the dashboard
- **Archived** — The suggestion was archived due to inactivity

This closes the loop for anyone who requested documentation updates via Slack, so they know when their request has been addressed.
...
3. Enable it to start receiving thread updates

Lifecycle notifications apply to all suggestions in your organization that originated from Slack threads.
