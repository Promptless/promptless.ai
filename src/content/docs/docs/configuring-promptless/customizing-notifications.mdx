...
For teams without a Slack or Teams channel, you can list one or more email addresses under `escalation.email` instead. Email is escalation-only—suggestion notifications never arrive by email—and it's the last-resort path for reaching a human when no chat channel is configured. Promptless emails one of the listed addresses, sent as Promptless, and it only ever sends to an address you've listed here. Presence works the same way as the channel fields: omit `email` to inherit, set an empty list to disable, or list addresses to route escalations there. Email escalation requires email delivery to be configured for your deployment, and it's available by default on Promptless Cloud.
