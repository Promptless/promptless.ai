...
Promptless sends notifications to your team when documentation suggestions are ready for review. You can customize these notifications by editing a configuration file in your organization's Long-Term Context, and configure additional notification types in Organization Settings.

## Suggestion Lifecycle Notifications

When you request documentation updates through Slack, Promptless can notify you when the status of your suggestion changes. Promptless posts a reply to your original Slack thread when the suggestion is:

- **Merged**: The docs PR was merged
- **Closed**: The docs PR was closed without merging
- **Rejected**: Someone rejected the suggestion in the dashboard
- **Archived**: The suggestion was automatically archived due to inactivity

This keeps you informed without checking the dashboard or watching the docs PR.

### Enabling Lifecycle Notifications

1. Go to **Organization Settings** in the Promptless dashboard.
2. Find the **Suggestion lifecycle notifications** toggle.
3. Enable it to start receiving status updates.

<Note>
  Lifecycle notifications are sent to the same Slack thread where you originally requested the documentation update. If no Slack thread is associated with a suggestion, the notification is skipped.
</Note>
