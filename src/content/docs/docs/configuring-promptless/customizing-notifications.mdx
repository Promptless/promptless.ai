...
<Note>
You can route notifications to any Slack channel by name, including one created after you connected Slack—Promptless resolves it automatically when you save the configuration that routes to it, so there's no need to reconnect or refresh your channel list first. To deliver messages there, though, Promptless must be a member of the channel. For a private channel, or a public channel the bot hasn't joined, invite it with `/invite @Promptless`. Otherwise Slack rejects the delivery and the notification doesn't arrive.
</Note>
