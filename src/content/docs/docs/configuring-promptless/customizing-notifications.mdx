...
Sometimes Promptless needs an answer from your team before it can continue—a clarifying question, or a decision it can't make on its own. Set `escalation_channel` in your policies to give those questions a dedicated Slack channel:
...
Escalation questions land in this channel even when you've turned suggestion notifications off or haven't configured a notification channel, so a blocking question never gets lost. The channel follows the same naming rules as `slack_channel` (use the name, with no `#` prefix) and is Slack-only. If you leave it unset, Promptless asks these questions through its normal notification routing instead.
