...
Sometimes Promptless needs an answer from your team before it can continue—a clarifying question, or a decision it can't make on its own. The `escalation` section is a sibling to `notification` with the same shape, so you can route those questions to a dedicated channel on Slack, Teams, or both:
