...
| Field | Description |
|-------|-------------|
| `auto_create_pr` | Automatically open documentation PRs for suggestions. Gates only automatic PR creation — see the note below |
| `auto_merge` | Automatically merge automatically created documentation PRs |
| `suppress_source_pr_comments` | Skip posting comments on source PRs |

<Note>
`auto_create_pr` gates only *automatic* PR creation. When it's off, Promptless leaves suggestions as drafts and doesn't open a PR on its own — but you can still open one on demand by asking the agent directly, using the "Create PR" button in Slack, or creating it from the dashboard. `auto_merge` applies only to PRs that Promptless creates automatically.
</Note>
