...
Follow-on suggestions inherit trigger keys from their lineage. When you request more work on an existing suggestion—for example, by commenting on its documentation PR to ask for a backport—any new suggestion created during that follow-on inherits the matched trigger keys from the originating event. As a result, `if.trigger` rules resolve consistently across a suggestion's full lineage, not just the trigger that fired first. A `doc_collection` condition always matches against each suggestion's own collection, so a collection-scoped rule never applies outside its collection.
