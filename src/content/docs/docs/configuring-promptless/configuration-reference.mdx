...
| Field | Description |
|-------|-------------|
| `slack_channel` | Slack channel name for notifications (no `#` prefix), or `false` to disable an inherited Slack channel |
| `msteams_channel` | Microsoft Teams channel conversation ID (e.g. `19:…@thread.tacv2`) for notifications, or `false` to disable an inherited Teams channel |
...
Each channel field is a tri-state value:

- A **channel name** (or Teams conversation ID) routes notifications there.
- **`false`** (unquoted) disables an inherited channel for that scope, overriding a channel set in `policies.default` or an earlier rule.
- **Omitting** the field inherits the channel from earlier overlays. A bare `null` also reads as inherit rather than disable, since once the configuration is materialized it's indistinguishable from an absent field. Use `false` to turn a channel off.

To target a Slack channel literally named `false`, quote it as `"false"`; the unquoted boolean is reserved for the disable sentinel. The `false` value applies to `slack_channel` and `msteams_channel` independently, so you can disable one channel while leaving the other inherited.
...
policies:
  ...
  rules:
    - if:
      ...
      then:
        notification:
          slack_channel: false   # silence Slack for this collection
```
