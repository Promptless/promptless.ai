...
slug: docs/configuring-promptless/configuration-reference
sidebar:
  hidden: false
  order: 7
---
...
All Promptless configuration lives in a single `promptless.yaml` file in your organization's Long-Term Context repository. This file defines your doc collections, triggers, context sources, and publishing policies in one git-versioned location.

## Configuration Editor

View and edit your configuration in the dashboard at [app.gopromptless.ai/configuration](https://app.gopromptless.ai/configuration). The Configuration page offers two editing modes: a structured form editor (default) and a YAML editor for advanced use cases.

### Structured Form Editor

The form editor organizes settings into four sections—Doc Collections, Context Sources, Triggers, and Policies—with form controls that guide you through valid options without requiring YAML knowledge.

- **Doc Collections** — Add repositories from your connected GitHub integrations. Each collection includes settings for platform, branch, docs framework, site URL, and directory scope.
- **Context Sources** — Configure read-only integrations (Jira, Confluence, Linear, Notion) with scoping options to limit which data Promptless can query.
- **Triggers** — Define which events initiate documentation work, with filters for repositories, branches, directories, and lifecycle events. Built-in triggers (mentions, API, web tasks) appear as read-only references—they're always active and can be used in policy rules.
- **Policies** — Set default notification and publishing behavior, then add rules that override the defaults for specific triggers or doc collections.

Click **Review changes** to see a diff of the YAML that will be committed, then **Save & commit** to apply.

### YAML Editor (Advanced)

Toggle to YAML mode for direct file editing with syntax highlighting, schema-driven completions, and inline validation. Use this mode when you need precise control over the file format or want to copy configurations between environments.

### Permissions and Conflict Detection

All organization members can view the configuration. Only admins can edit and save changes—non-admins see a read-only view.

Both editors use compare-and-swap to prevent overwriting concurrent changes. If someone else modifies the configuration while you're editing, you'll see a conflict warning and can reload to get the latest version.
...
These broad defaults let Promptless start listening for documentation-worthy events immediately. You can edit your configuration anytime to narrow scope—for example, restricting triggers to specific repositories or limiting context sources to particular projects.
...
context_sources:   # Read-only integrations for additional context
...
Context sources give Promptless read-only access to your organization's tools. They're used for narrowing scope when you want to limit which projects, spaces, or databases Promptless can query. Each entry requires a `source_type` field.
...
<AccordionGroup>
  ...
  <Accordion title="msteams_listen">
...
These updates are committed directly to your Long-Term Context repository.
...
- Auto-publish and notification preferences
