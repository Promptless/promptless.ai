...
| Field | Description |
|-------|-------------|
| `slack_channel` | Slack channel name for escalation questions (no `#` prefix) |
| `msteams_channel` | Microsoft Teams channel conversation ID (e.g. `19:…@thread.tacv2`) for escalation questions |
| `email` | List of email addresses for escalation questions (escalation only; not used for suggestion notifications) |

`escalation` is a sibling section to `notification` with the same shape. It covers the questions Promptless asks when it needs a decision from your team before it can proceed. Configure it on Slack, Teams, or both, exactly as you configure `notification`. Unlike suggestion notifications, escalation questions reach the escalation channel(s) even when normal notifications are off or no notification channel is set, so a blocking question always gets to someone. When a platform's escalation channel is unset, Promptless asks these questions through your normal notification routing on that platform.

`escalation.email` is a list of email addresses rather than a single channel, and it applies to escalation only—suggestion notifications never go out by email. It's the reach-a-human path for teams without a Slack or Teams channel: when Promptless hits a blocking question and has no chat channel to post it in, it emails one of these addresses instead, sent as Promptless. The addresses you list are the only ones Promptless can email, and any address that isn't listed is rejected. Presence works the same way as the channel fields: omit the field to inherit, set an empty list to disable inherited addresses, or list addresses to route there. Email escalation requires email delivery to be configured for your deployment, and it's available by default on Promptless Cloud.
