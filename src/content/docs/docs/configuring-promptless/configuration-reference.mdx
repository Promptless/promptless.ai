...
Edit your configuration in the dashboard at [app.gopromptless.ai/configuration](https://app.gopromptless.ai/configuration). The editor provides:

- YAML syntax highlighting and schema-driven completions
- Inline validation with line-specific error markers
- Conflict detection that prevents overwriting concurrent changes
- Direct commits to your Long-Term Context repository
...
Context sources give Promptless read-only access to your organization's tools. They're used for narrowing scope when you want to limit which projects, spaces, or databases Promptless can query.
...
| Source | Scope Field | Description |
|--------|-------------|-------------|
...
Triggers define events that automatically initiate documentation work. These are passive intake events that require explicit configuration.
...
<AccordionGroup>
  <Accordion title="github_pr">
    ...
    | `trigger_on` | Events that fire: `opened`, `first_approval`, `merge` |
  ...
  <Accordion title="github_commit">
    ...
    | `branches` | Branches to monitor (defaults to default branch) |
    | `trigger_directories` | Only trigger when changes touch these directories |
    | `repo_topics` | Only trigger for repos with these GitHub topics |
  </Accordion>
  ...
  <Accordion title="gitlab_mr">
    ...
    | Field | Description |
    |-------|-------------|
    | `repos` | List of GitLab projects to monitor |
    | `excluded_repos` | Projects to exclude |
    | `trigger_on` | Events that fire: `opened`, `merge` |
    | `trigger_directories` | Only trigger when changes touch these directories |
  ...
  <Accordion title="bitbucket_pr">
    ...
    | Field | Description |
    |-------|-------------|
    | `repos` | List of Bitbucket repositories to monitor |
    | `excluded_repos` | Repositories to exclude |
    | `trigger_on` | Events that fire: `opened`, `merge` |
    | `trigger_directories` | Only trigger when changes touch these directories |
    | `branches` | Only trigger for PRs targeting these branches |
  ...
  <Accordion title="slack_listen">
    ...
    | Field | Description |
    |-------|-------------|
    | `channels` | List of channel names to monitor (no `#` prefix) |
  ...
  <Accordion title="msteams_listen">
    ...
    | Field | Description |
    |-------|-------------|
    | `channel_ids` | List of Teams channel conversation IDs to monitor |
  </Accordion>
  ...
  <Accordion title="clickup">
    ...
    | Field | Description |
    |-------|-------------|
    | `statuses` | List of task statuses that trigger documentation work |
  </Accordion>
</AccordionGroup>
