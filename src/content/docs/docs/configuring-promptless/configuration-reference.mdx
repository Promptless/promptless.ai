...
policies:
  default:
    notification:
      ...
      escalation_channel: docs-escalations
    publishing:
      auto_create_pr: true
      auto_merge: false
...
| Field | Description |
|-------|-------------|
| `slack_channel` | Slack channel name for notifications (no `#` prefix) |
| `msteams_channel` | Microsoft Teams channel conversation ID (e.g. `19:…@thread.tacv2`) for notifications |
| `escalation_channel` | Slack channel name for the agent's escalation and clarifying questions (no `#` prefix) |
...
The optional `escalation_channel` is a separate Slack channel for the questions Promptless asks when it needs a decision from your team before it can proceed. Unlike suggestion notifications, escalation questions are delivered here even when normal notifications are off or no notification channel is set, so a blocking question always reaches someone. It follows the same naming rules as `slack_channel` (channel name, no `#` prefix) and is Slack-only. When unset, Promptless routes these questions through its normal notification settings.
...
- **Slack channel renames**: Channel names in `slack_listen` triggers and `notification.slack_channel` / `notification.escalation_channel` policies are updated.
