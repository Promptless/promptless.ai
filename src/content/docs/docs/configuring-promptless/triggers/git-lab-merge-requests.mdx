...
Use the `trigger_directories` field to only trigger when changes are made to specific directories. When specified, only merge requests that contain changes to those directories will be considered.

## Auto-publish Mode

When auto-publish is enabled in your [publishing policies](/docs/configuring-promptless/configuration-reference#policies), Promptless automatically creates a new PR in your documentation repository with suggested changes.
