...
4. When reviewing the suggestion in the Promptless app, you see the added images at the bottom of the review interface, allowing you to approve or reject their inclusion.
...
Promptless tries to reason about whether each message in a thread is actionable—not every reply triggers a documentation update. However, if you want to explicitly tell Promptless to ignore a message, start it with `aside` or `/aside`:
