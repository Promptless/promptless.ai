...
API triggers allow external systems to request documentation updates from Promptless. This enables integration with CI/CD pipelines, custom automation tools, or any system that can make HTTP requests.

## Use Cases

API triggers are useful when you want to:
...
```
POST /organizations/{org_id}/triggers/api
```

Replace `{org_id}` with your organization ID, which you can find in your dashboard URL or organization settings.
...
```bash
curl -X POST "https://app.gopromptless.ai/api/v1/organizations/{org_id}/triggers/api" \
...
```json
{
  ...
  "job_id": "job-456-...",
  "matched_pipeline_ids": ["pipeline-789-..."]
}
```
...
View all API triggers on the [Triggers page](https://app.gopromptless.ai/triggers). API triggers display the submitted instructions and any context you included in the request.

### Change History

Filter by "API" source in [Change History](https://app.gopromptless.ai/change-history) to see documentation suggestions that originated from API requests.
