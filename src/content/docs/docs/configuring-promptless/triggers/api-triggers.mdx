...
| `doc_collection_id` | string | No | Target a specific doc collection by its ID. When provided, the trigger only routes to that doc collection instead of all doc collections in your organization. |
...
### Targeting a Specific Doc Collection

If your organization has multiple doc collections, you can target a specific one using the `doc_collection_id` parameter. Without this parameter, the trigger routes to all doc collections in your organization.
...
curl -X POST "https://api.gopromptless.ai/triggers" \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "instructions": "Update the API reference for the new endpoints",
    "doc_collection_id": "your-collection-id"
  }'
```

You can find your doc collection ID in the URL when viewing a doc collection in the dashboard, or through the dashboard settings.
...
| 409 | `org_not_configured` | Your organization hasn't finished setting up Promptless—no doc collections are ready to receive triggers |
| 409 | `no_eligible_doc_collection` | The `doc_collection_id` you specified isn't configured, doesn't belong to your organization, or isn't ready |
| 422 | Validation error | The request body is invalid (e.g., empty instructions or a malformed `doc_collection_id` UUID) |
...
2. **Routing**: The request is routed to your configured doc collections—either all of them, or just the one you specified with a `doc_collection_id`.
