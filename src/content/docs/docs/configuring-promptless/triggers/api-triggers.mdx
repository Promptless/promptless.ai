...
| `doc_collection_id` | string | No | Target a specific doc collection by its ID. When provided, the trigger only routes to API projects connected to that collection instead of all API projects in your organization. |
...
If your organization has multiple doc collections, you can target a specific one using the `doc_collection_id` parameter. Without this parameter, the trigger routes to all API projects in your organization.
...
| 409 | `no_eligible_pipelines` | No API projects are configured for your organization, or the specified `doc_collection_id` has no associated API project |
...
2. **Routing**: The request is routed to your API projects—either all of them, or just those connected to a specific doc collection if you provided a `doc_collection_id`
