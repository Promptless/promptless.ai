...
A direct @mention works even when the comment comes from an automated account. If a CI or review GitHub Action—for example, one that runs Diataxis, Doc Detective, snippets, or AGENTS.md checks—leaves a PR comment that tags Promptless, that comment triggers documentation work just like a comment from a person. Promptless still ignores bot comments that don't mention it.
