...
You can request documentation updates by @mentioning Promptless in a pull request's title or description. When a PR opens with an `@promptless` mention, Promptless treats it as an explicit opt-in and triggers a documentation review—even if your project is configured for "first approval" mode.

### How to Use

Include `@promptless` anywhere in your PR title or description:
...
PR description mentions are useful when:

- Your project uses "first approval" mode but you want an earlier review for this PR
- External tools (like Mintlify or automated PR openers) create PRs that need documentation
- You want to add documentation requests to your PR template so every PR is reviewed
- You want to explicitly request documentation without waiting for approval

### How It Works

When a PR with an `@promptless` mention opens:

1. The mention overrides your project's trigger mode—the PR is reviewed on open
2. Directory filters still apply; repository scope filters do not (see below)
3. Promptless creates documentation suggestions if changes are relevant

#### PRs opened against your docs repository

When you open a PR directly against your documentation repository with an `@promptless` mention, Promptless triggers a review even if that repository is not in your project's source scope. Docs repositories are typically excluded from source triggers during onboarding to avoid triggering on routine documentation edits. The mention serves as an opt-in for a documentation review.
...
Promptless recognizes these mention patterns:

- `@promptless`
- `@Promptless` (case-insensitive)
- `@promptless-for-oss`
- `@promptless,` or `@promptless.` (with trailing punctuation)

Email addresses like `support@promptless.ai` and partial matches like `@promptlessly` are not recognized as mentions.

### Requirements

- The GitHub integration must be installed for your repository
- For PRs on source repositories: the repository must be in scope for at least one of your projects
- For PRs on docs repositories: no source scope requirement—the mention itself acts as the opt-in
...
An `@promptless` mention doesn't have to come from a person. A direct mention takes priority over Promptless's bot filter, so a comment from an automated account—such as a GitHub Actions workflow—triggers documentation work just like a comment from a teammate. This lets you build documentation requests and reviews into your CI: have an Action post an `@promptless` comment whenever you want docs updated or reviewed.
