...
- Fixes findings according to your config's `MinAlertLevel`: when you set an alert level, Promptless fixes every finding Vale reports; when it's unset, Promptless fixes error-severity findings and uses judgment on warnings and suggestions
- Reproduces your repository's Vale CI invocation on the changed files and confirms they pass before creating the suggestion
...
1. **Sync packages** - Downloads any external style packages your config references. Promptless skips this step when those styles are already vendored under your config's `StylesPath`, so it never overwrites your checked-in style files.
2. **Lint prose** - Runs Vale on created or edited prose files using your config.
...
When your config doesn't set `MinAlertLevel`, Promptless falls back to error-first handling:
...
In this mode, error-severity rules are hard constraints, while Promptless treats warning and suggestion rules as guidance and balances them against the established voice and conventions in your docs.
