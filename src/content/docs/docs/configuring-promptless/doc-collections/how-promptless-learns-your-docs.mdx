...
Place skill files in `.claude/skills/` or `.agents/skills/` in your docs repository. Each skill is a directory containing a `SKILL.md` file with YAML frontmatter:
...
When Promptless starts a documentation task, it scans these directories for skills. If a skill's description matches what the agent is about to do, it invokes that skill instead of deriving the workflow from scratch.
