...
When those assertions are only reviewed by humans, drift is easy to miss. A product change lands, a workflow moves, or a selector changes, and the first person to run the stale instructions may be a user.

Promptless uses [Doc Detective](https://docs.doc-detective.com/) to make those assertions executable. This is Promptless's implementation of the [Docs as Tests](https://www.docsastests.com/docs-as-tests/concept/2024/01/09/intro-docs-as-tests.html) framework: documentation should be tested against the real product experience, not just proofread for style or checked for broken links.
...
Promptless stores two Doc Detective settings on each doc collection:

- `use_skill_doc_detective`, which controls whether the workflow is enabled
- `doc_detective_config_path`, which points to the collection's Doc Detective config file when one is configured

When a documentation session starts for an enabled collection, Promptless prepares the agent environment for Doc Detective work:

- It injects a `promptless-doc-detective` skill into the session so the agent knows when and how to add or update Doc Detective coverage.
- It writes `working_notes/doc_detective_config.md`, which tells the agent whether a config path is configured and whether that file exists in the current checkout.
- It enables the upstream `doc-detective@doc-detective` plugin from the `doc-detective/agent-tools` marketplace in the active Claude session settings.
- It instructs the agent to inspect existing Doc Detective specs and nearby config files before creating new coverage.

The key behavior is that Doc Detective coverage is treated as part of done when a suggestion changes documented workflows, UI selectors, navigation, executable step sequences, or other content that users are expected to follow.
