...
You can give doc collections custom display names to make them easier to identify at a glance. This is especially helpful when you have multiple collections from similar repositories or want to distinguish between internal and external docs.

Set the display name when creating a doc collection, or edit it later in the settings modal. Without a display name, the doc collection shows the repository identifier (e.g., `acme/docs (github)`) as the default label.
