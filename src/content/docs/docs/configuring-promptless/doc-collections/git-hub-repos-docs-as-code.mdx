...
### Dashboard fallback for private messages

Reviewers without access to a private Slack conversation see Slack's "There's been a glitch" error when clicking the trigger link. For DMs and private group messages, Promptless adds a second link to the suggestion's Triggers & Analysis tab in the dashboard. Anyone with dashboard access can read the full message there, regardless of their Slack permissions.
