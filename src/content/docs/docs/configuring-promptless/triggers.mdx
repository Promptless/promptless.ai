...
Open the **Backfill suggestions** panel on a trigger row, choose a window of the **last 14 days**, **last 30 days**, or **last 90 days**, and select **Preview**. The preview lists every pull request the trigger would have matched in that window, using the same repository, directory, and branch filters as the live webhook. Each result links to the pull request and shows whether it's open or merged.
