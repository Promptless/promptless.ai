...
A proposed fix opens in the finding's destination repository. That destination is the selected instruction repository chosen for its GitHub issue. It requires that repository to have **Propose Remediations** enabled in PIG Settings.
