...
description: Enable optional trace collection, authorize a Claude, Codex, or Cursor host, and verify that its sessions reach your trace analyzer.
...
This guide continues the Acme example. It covers Claude Code, Codex, and Cursor; the Claude collector can also discover Claude Desktop traces on the same machine. Gemini plugins do not currently collect native traces.
...
<Steps>
  1. **Enable collection in the hub**
     ...
     The compiler adds the host runtime and collection hooks to the literal `pig` plugin for Claude, Codex, and Cursor. It does not add them to `docs` or `dev`, and it does not provision an analyzer. With the setting omitted or `false`, generated runtime files and collection hooks are absent.

     Cursor collection also depends on your analyzer. The default enrollment policy adds Cursor after the analyzer reports `cursor` support, and removes it if a later check-in drops that support.
