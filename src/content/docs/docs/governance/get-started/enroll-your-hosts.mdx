...
Once a host installs your Claude or Codex plugins, the injected [host runtime](/docs/governance/start-here/key-concepts) (`promptless-host-runtime`) is present and ready to enroll with your worker and collect that host's session traces.

## Point the host at your worker

The host runtime reads the worker base URL from the `PROMPTLESS_WORKER_BASE_URL` environment variable. A self-hosted deployment must set `PROMPTLESS_WORKER_BASE_URL` on each host to your own worker's hostname — the hostname you expose in [Deploy the analyzer worker](/docs/governance/deploy-the-worker/deploy-the-analyzer-worker) — before you enroll that host. The compiled-in default points at Promptless's own hosted worker, so it is not a general-purpose fallback for a self-hosted deployment.
...
Enrollment uses a browser approval, so a signed-in member of your Promptless organization authorizes each host. Any signed-in member can approve a host — org-admin is not required — and the approval is attributed to the approving user, recorded server-side as who approved that host.
