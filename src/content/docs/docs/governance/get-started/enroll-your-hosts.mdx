...
<Steps>
  ...
  3. **Approve the host**
     ...
     Start a new Claude Code or Codex session. The generated startup hook initializes the local runtime and starts enrollment when needed. In the browser, sign in to Promptless and approve the host. Any signed-in organization member can approve; organization administrator access is not required.

     Before approval, the enrollment page determines which organization to enroll the host into. When you belong to more than one organization, the page shows a searchable list of them. Select the organization to enroll the host into, then select **Continue**. If the host's deployment isn't in the organization you selected, the page tells you. Select a different organization and select **Continue** again, without leaving the page. Once the deployment matches, the flow continues to approval.
...
| Trace is stored but analysis does not complete | Ask the operator to check repository binding, quiet-period eligibility, model configuration, and the analysis queue                 |
