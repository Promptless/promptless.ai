...
Host enrollment connects a person's agent installation to your trace analyzer. Instructions work without enrollment. Enable collection when you want PIG to analyze how those instructions perform in real sessions. Collection here is a host-side control; an admin [enables and pauses analysis](/docs/governance/deploy-the-worker/enable-trace-analysis) separately, per worker installation.
