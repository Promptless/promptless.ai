...
Sign in to the [Promptless Dashboard](https://app.gopromptless.ai/instruction-hub/settings/workers) as an organization admin and select the organization that will own the analyzer.

<Steps>
  1. In the PIG sidebar, open **Settings → Workers**. Under **Add analyzer**, enter a name such as `production-us-east-2` and select **Add analyzer**.
  2. Copy the credential into your secret manager. It is shown once and cannot be retrieved after you dismiss it, leave the **Workers** page, or switch organizations.
  3. Deliver the credential through the Kubernetes Secret referenced by `spec.hosted.installTokenSecretRef`. Follow the installation instructions for your cluster.
</Steps>
...
After the analyzer checks in, an organization administrator chooses which repositories it reads in [Select instruction repositories](/docs/governance/deploy-the-worker/deploy-the-analyzer-worker#5-select-instruction-repositories). Instruction repositories are selected on the PIG **Workers** settings page, not configured in Helm.
