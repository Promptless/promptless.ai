...
Sign in to the [Promptless Dashboard](https://app.gopromptless.ai/pig/settings/workers) as an organization admin and select the organization that will own the analyzer.

<Steps>
  1. In the PIG sidebar, open **Settings → Workers**. Under **Add analyzer**, enter a name such as `production-us-east-2` and click **Add analyzer**.
...
After the analyzer checks in, an organization administrator chooses which repositories it reads in [Select instruction repositories](/docs/governance/deploy-the-worker/deploy-the-analyzer-worker#5-select-instruction-repositories). Instruction repositories are selected in PIG Settings, not configured in Helm.
