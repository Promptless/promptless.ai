...
- The public worker chart and a [registration download from PIG Settings](/docs/governance/deploy-the-worker/plan-your-deployment#register-an-analyzer). Map its `runtime_url` to `instructionHub.runtimeBaseUrl`, `deployment_id` to `instructionHub.deploymentInstanceId`, and `install_token` to the install-token Secret key. Obtain the configuration hash for your manual deployment from Promptless; the registration download does not include it.
...
<Steps>
  1. **Prepare the namespace, identity, and secrets.** Confirm the cluster you intend to change:
     ...
     | Key                         | Value                                                                                                |
     | --------------------------- | ---------------------------------------------------------------------------------------------------- |
     | `install-token`             | `install_token` from your registration download. This is separate from a host enrollment credential. |
