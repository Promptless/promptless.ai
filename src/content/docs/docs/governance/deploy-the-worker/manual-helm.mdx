...
This guide uses the worker chart and image version `0.2.0`, for operator-managed releases. Your operations team schedules and applies each upgrade. For automatic updates with pause and version-pin controls, use the [default Helm installation](/docs/governance/deploy-the-worker/deploy-the-analyzer-worker).
...
The complete example below uses Acme's private GitHub hub and an EKS service account with an IAM role for S3. On other clusters, arrange AWS credential delivery with your platform team before installing. Chart `0.2.0` has no generic `env`, `envFrom`, or S3 credential-Secret setting.
...
<Steps>
  ...
  2. **Configure the worker.** Save this as `values.yaml`. Replace every `REPLACE_` value, the example AWS account, and the hostname with your environment's values. Get the numeric repository ID with `gh api repos/acme/acme-instruction-hub --jq .id` using an account that can read the hub. Choose a model name your provider account can use.
     ...
     For an ingestion-only pilot, leave `instructionHub.analysis.activationAt` empty. The chart then omits model and repository settings from the worker. Complete all analysis settings before enabling it. See the [manual Helm reference](/docs/governance/deploy-the-worker/manual-helm-reference) for supported providers and value mappings.
...
Follow [Verify your deployment](/docs/governance/deploy-the-worker/verify-your-deployment). Use `deployment/instruction-hub-worker` for analyzer log commands. The same enrollment, storage, analysis, and dashboard checks apply to this installation.
