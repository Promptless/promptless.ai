...
<Steps>
  ...
  3. **Render and install.** Download the pinned public worker chart, then render it to catch missing required values without changing the cluster:
     ...
     <Aside type="caution">
       Helm's `--atomic` handles failed Kubernetes release changes. It does not undo database migrations or restore trace objects. Back up your database before upgrades and confirm the previous application version supports the resulting schema before rolling it back.
     </Aside>
</Steps>
