...
<Aside type="note">
  A self-service **Add deployment** setup page in hosted Promptless is forthcoming. It will issue the install token, generate a values file, and publish the chart to a Helm repository. Until it ships, Promptless provides your `plih_` install token and the chart directly, and you install the chart as shown below.
</Aside>
