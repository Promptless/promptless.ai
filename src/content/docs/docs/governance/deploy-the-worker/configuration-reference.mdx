...
<Aside type="note">
  The [supervisor configuration](/docs/governance/deploy-the-worker/install-the-supervisor) is a planned interface, pending implementation. Its `PIGDeployment` fields are not Helm values and cannot be applied to today's worker chart.
</Aside>
...
Render the chart with your proposed values and review the generated references and endpoints. Apply through the deployment's single owner: Helm, GitOps, or the future supervisor. Do not let two controllers manage the same worker resources.

After rollout, repeat the [complete-session verification](/docs/governance/deploy-the-worker/deploy-the-analyzer-worker#4-verify-a-complete-session). Configuration validation and a healthy pod do not prove that credentials can access the database, bucket, repository, or model.
