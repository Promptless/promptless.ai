...
| `INSTRUCTION_HUB_ANALYSIS_MIRROR_ROOT`          | Local root for the analyzer's repository mirror. Defaults to `/var/lib/instruction-hub/analysis-mirrors`. The chart backs it with ephemeral pod-local storage (an `emptyDir`), not a persistent volume. The mirror is re-cloned on pod restart, and no PersistentVolume is required. Because the worker runs with a read-only root filesystem, an override must resolve under the `/var/lib/instruction-hub` mount the chart provides for the mirror. A path elsewhere on the root filesystem is not writable. |
