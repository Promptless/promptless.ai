...
This page describes the current worker chart, version `0.2.0`. Helm values become environment variables in the worker. Start with [Install with Helm](/docs/governance/deploy-the-worker/deploy-the-analyzer-worker) for a complete values file.

<Aside type="note">
  The [supervisor configuration](/docs/governance/deploy-the-worker/install-the-supervisor) is a proposed interface, pending implementation. Its `PIGDeployment` fields are not Helm values and cannot be applied to today's worker chart.
</Aside>
...
The chart uses the AWS SDK's normal credential discovery. It has no generic environment-variable passthrough or explicit AWS credential-Secret setting. EKS workload identity can supply credentials through the service account. For other arrangements, coordinate credential injection with your platform team. Azure Blob Storage and Google Cloud Storage are not current storage backends; contact [Promptless](mailto\:help@gopromptless.ai) if you need them.
...
Sentry uses a separate Secret reference. See [Observability](/docs/governance/deploy-the-worker/observability) for the exact values.

## Validate a configuration change

Render the chart with your proposed values and review the generated references and endpoints. Apply through the deployment's single owner: Helm, GitOps, or the proposed supervisor. Do not let two controllers manage the same worker resources.

After rollout, repeat the [complete-session verification](/docs/governance/deploy-the-worker/deploy-the-analyzer-worker#verify-a-complete-session). Configuration validation and a healthy pod do not prove that credentials can access the database, bucket, repository, or model.
