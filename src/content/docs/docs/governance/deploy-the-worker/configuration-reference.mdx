...
description: Every environment variable that configures the instruction-hub-worker — the required deployment values and the analysis settings that turn on the Friction Analyzer.
...
The `INSTRUCTION_HUB_ANALYSIS_*` variables turn on and configure the [Friction Analyzer](/docs/governance/start-here/key-concepts). Set them together — the analyzer needs its model and repository configured before it runs.
...
| `INSTRUCTION_HUB_ANALYSIS_REPOSITORY_TOKEN`     | A repository-scoped token the remediation flow uses to open pull requests, and that the analyzer uses to clone and read your Instruction Hub for its local mirror. Optional for public repositories.                                                                                   |
...
| `INSTRUCTION_HUB_ANALYSIS_MIRROR_ROOT`          | Local root for the analyzer's repository mirror. Defaults to `/var/lib/instruction-hub/analysis-mirrors`. The chart backs it with ephemeral pod-local storage (an `emptyDir`), not a persistent volume, so the mirror is re-cloned on pod restart and no PersistentVolume is required. |
