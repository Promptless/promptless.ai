...
<Aside type="note" title="Configuration scope">
  This reference covers the worker Helm chart. The [supervisor configuration](/docs/governance/deploy-the-worker/install-the-supervisor) uses `PIGDeployment` fields, which are not worker chart values.
</Aside>
...
Render the chart with your values and review the generated references and endpoints. Apply through the deployment's single owner: Helm, GitOps, or the supervisor. Do not let two controllers manage the same worker resources.
