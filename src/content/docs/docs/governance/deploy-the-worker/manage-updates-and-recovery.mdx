...
If a release needs a cloud change, PIG reports `Blocked=True` with the unmet requirement and required operator action. Review and apply the change through Terraform, then let PIG rerun its checks. An unpaused deployment resumes its stable-release transition once the requirement is satisfied.
...
2. Check hosted compatibility, storage access, model and repository requirements, and available capacity.
3. Check the release's recovery prerequisites, then run its coordinated schema migrations. Your backup workflow creates and retains cloud recovery points; provide any release-required recovery confirmation before a destructive migration.
