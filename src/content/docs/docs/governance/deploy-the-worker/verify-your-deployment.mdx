...
Confirm `trace_object_status = 'written'` and an ingestion timestamp matching your pilot. To inspect the persisted storage fields for that session:

```sql
BEGIN READ ONLY;
SELECT t.id, field.key, field.value
FROM agent_traces AS t
CROSS JOIN LATERAL jsonb_each(to_jsonb(t)) AS field
WHERE t.source = 'codex' AND t.session_id = 'REPLACE_SESSION_ID'
  AND field.key LIKE 'trace_object_%'
...
<Tabs>
  <TabItem label="S3">
    Confirm the recorded bucket and key with your normal S3 inspection tools. Worker chart `0.2.0` records the canonical location in `agent_traces.trace_object_s3_uri`.
  </TabItem>
