...
Deployment verification and acceptance require a healthy analyzer whose last check-in reached Promptless less than 15 minutes ago. Promptless measures staleness using the time it received that check-in. A running pod can still appear stale if its check-ins stop reaching Promptless.
