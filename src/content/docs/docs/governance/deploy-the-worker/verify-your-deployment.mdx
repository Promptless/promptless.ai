...
Verification and acceptance require the worker's reported status to be healthy and not stale. Promptless judges staleness from the check-in time it last recorded for the worker, not from the worker's self-reported timestamp. It marks the worker stale once that recorded check-in is 15 minutes old. A worker whose pod is running locally can still report as stale when its check-ins stop reaching Promptless. A stale worker fails these checks.
