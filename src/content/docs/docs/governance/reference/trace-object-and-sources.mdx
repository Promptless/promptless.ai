...
The collector supports four source families. Startup and terminal hooks can supply a transcript path directly. Background catch-up also discovers files in the following locations:
...
| Cursor         | `cursor`         | Desktop transcripts under `~/.cursor/projects/*/agent-transcripts/**/*.jsonl`, plus the Cursor session database (see platform paths below)             |
...
For Cursor, the collector reads the desktop session database. Its default location depends on the operating system, and `PROMPTLESS_CURSOR_DATABASE` overrides it:

| Platform | Cursor session database                                              |
| -------- | -------------------------------------------------------------------- |
...
Claude Code's generated hooks also attempt local Claude Desktop collection. Codex and Cursor use their own generated hooks. Gemini currently distributes instructions without native trace collection. See [Supported agents](/docs/governance/reference/supported-agents) for the distinction.
