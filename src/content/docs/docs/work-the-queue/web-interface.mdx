...
When a suggestion's branch matches its base branch, the viewer shows an explanatory panel instead of a bare message. The panel heading reads "No changes against" plus the base branch name. If Promptless can't determine the base branch name, the heading uses a generic reference to the base branch instead.
