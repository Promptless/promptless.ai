...
- **Suggestion created** or **Suggestion updated**: A row for each documentation suggestion the trigger produced, with a chip showing where that suggestion's pull request stands: **PR Open**, **Published** (the PR merged), or **Closed** (the PR was closed without merging). Click a suggestion to open it. When a run finishes without producing a suggestion, the timeline notes **No suggestion created** instead.
