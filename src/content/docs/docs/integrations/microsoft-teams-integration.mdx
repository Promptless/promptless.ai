...
## Enabling full features (channel visibility)

Once Promptless is connected to your Microsoft tenant, an administrator can optionally **Enable full features** from the Teams integration card on the [Promptless integrations page](https://app.gopromptless.ai/integrations). This grants Promptless read-only access to your teams and channels through Microsoft Graph, so it can sync the teams and channels available to the administrator who authorizes it—not just the channels Promptless has already been mentioned in.

A Microsoft tenant administrator must grant consent to enable full features. The consent applies org-wide, but the channels Promptless can see are scoped to that administrator's own memberships: admin consent grants an org-wide permission, not org-wide visibility. Promptless doesn't sync private and shared channels the authorizing administrator isn't a member of.

After you enable full features, the synced channels populate a picker when you choose a Microsoft Teams notification channel in your configuration policies. If a channel hasn't synced yet, you can still paste its conversation ID directly.
