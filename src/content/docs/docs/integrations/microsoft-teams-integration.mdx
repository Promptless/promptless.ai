...
When you message @Promptless in a personal (1:1) chat, Promptless reads the files and images you attach and uses them as context for the documentation it drafts, the same way it does in Slack. Inline images you paste into a message are read as well, in any conversation.

Files attached in Teams **channels and group chats** aren't read yet. Promptless surfaces them as unavailable while support for reading them is in progress, which depends on additional Microsoft Graph permissions that a Microsoft tenant administrator grants. Inline images still work in those conversations.
