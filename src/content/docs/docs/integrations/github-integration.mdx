...
To keep Promptless working with an IP allow list enabled, choose one of the following in your GitHub organization's IP allow list settings (**Settings → Authentication security**):
...
- **Add Promptless's egress IP address**: Add `3.143.177.103` to your allow list. This is the NAT gateway address that Promptless's requests originate from.

This applies to standard GitHub.com organizations. If you use GitHub Enterprise, see the [GitHub Enterprise Setup Guide](/docs/integrations/github-enterprise-integration) for the full list of addresses to allow.
...
<AccordionGroup>
  <Accordion title="Promptless shows 'no repositories accessible' during setup—what do I do?">
