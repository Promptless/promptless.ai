...
When selecting a docs repository during setup, Promptless shows which GitHub organization is connected above the repository picker. If Promptless doesn't have access to any repositories in your organization, you'll see an error message with a **Manage repository access** button that opens GitHub's installation settings directly.
