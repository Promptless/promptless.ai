...
Where Promptless raises serious configuration problems on your side that block it from doing its best work—for example, a test account that lacks the feature flags to reach a screen it needs to screenshot, or a Jira integration that isn't connected (or is connected, but that project isn't accessible) so it can't read a linked issue. These are blockers only your team can clear, not clarifying questions or status updates. See [Escalation channel](/docs/tune/notifications#escalation-channel) for the full behavior.
...
Escalations reach the escalation channel(s) even when normal notifications are off or no notification channel is set, so a blocker always gets to someone. When a platform's escalation channel is unset, Promptless raises these through your normal notification routing on that platform.

`escalation.email` routes escalations to a list of email addresses—useful for teams without a Slack or Teams channel. It's escalation-only: suggestion notifications never go out by email, and Promptless only emails addresses you've listed. Presence works like the channel fields: absent inherits, an empty list disables, a list routes there. Email escalation requires email delivery to be configured for your deployment. It's enabled by default on Promptless-hosted deployments; self-hosted deployments need to configure it.
