...
<details>
  ...
  **Match clause fields:**
  ...
  An optional top-level `instructions` field (a sibling of `match`) gives the trigger standing instructions passed to Promptless on each run; a present-but-blank value is rejected. The trigger creates a reply obligation on the ticket, like the built-in `jira_mention`—Promptless comments and owes an answer, tracked for follow-up.
</details>
...
<details>
  ...
  **Match clause fields:**
  ...
  Takes the same optional `instructions` field as `jira_new_issue`, and likewise creates a reply obligation on the ticket.
</details>
