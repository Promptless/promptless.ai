...
- **Inherit from default** keeps whatever channel the rule would otherwise inherit from `policies.default` or an earlier rule (the field is omitted).
- **Disabled — no Slack notifications** (or **no Teams notifications**) turns the inherited channel off for that scope, so Promptless sends nothing. This is the Form-mode equivalent of setting the field to `null` in YAML (`slack_channel: null` or `msteams_channel: null`).
- A **channel name** routes announcements to that channel.

The default policy has nothing to inherit, so its channel dropdowns offer **No Slack notifications** / **No Teams notifications** instead of Inherit and Disabled. Leaving one unset simply means Promptless doesn't announce on that platform.

The Slack dropdown lists the channels the Slack integration can see. The Teams dropdown lists the channels Promptless has seen activity in; when none are available, it falls back to a field where you paste a channel's conversation ID. That paste fallback can inherit or route to a channel, but it can't disable an inherited Teams channel—switch to YAML mode and set `msteams_channel: null` for that.
