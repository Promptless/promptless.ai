...
| `email`           | List of email addresses for escalation alerts, or `[]` to disable an inherited list |

Set any combination of the three; if none is configured, escalation alerts aren't sent and those problems surface only in the dashboard. The same presence semantics as `notification` apply (omit to inherit; `null` for `slack_channel`/`msteams_channel` or `[]` for `email` to disable), and `msteams_channel` carries the same Teams-activity prerequisite as `notification`.
