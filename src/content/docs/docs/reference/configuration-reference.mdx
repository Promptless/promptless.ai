...
The escalation channel is where Promptless routes operational alerts—such as [proactive notifications when a connected integration develops a problem](/docs/reference/integrations#proactive-notifications)—rather than suggestion announcements. It's a sibling of `notification` and, unlike `notification`, also accepts `email`.
...
| Field             | Description                                                                                                            |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `slack_channel`   | Slack channel name (no `#` prefix), or `null` to disable an inherited Slack channel                                    |
...
| `email`           | List of email addresses, or `[]` to disable inherited addresses. Email escalation is accepted only under `escalation`. |

Set any combination of the three. If none is configured, escalation alerts don't go out—those problems surface only in the dashboard. Each field follows the same presence semantics as `notification` (omitting inherits; `null` or `[]` disables), and `msteams_channel` has the same prerequisite: the Teams channel needs prior Promptless bot activity before alerts can be delivered there.
