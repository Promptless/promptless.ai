...
<details>
  ...
  Activates when a Jira issue matches a configured project, label, status, or status category—on the transition into that match.

  **Match clause fields:**

  | Field               | Description                                                                                                                             |
  | ------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
  | `projects`          | Jira project keys to match (for example `DOCS`)                                                                                         |
  | `labels`            | Issue labels to match                                                                                                                   |
  | `statuses`          | Issue statuses to match                                                                                                                 |
  | `status_categories` | Jira status categories to match (`new`, `indeterminate`, `done`) — stable across projects and renames, unlike per-project status names. |

  At least one of `projects`, `labels`, `statuses`, or `status_categories` must be set in each clause. Fields within a clause are ANDed and clauses are ORed, and all match case-insensitively.

  A `jira` trigger also accepts an optional top-level `instructions` field (a sibling of `match`) that gives the trigger standing instructions passed to Promptless on each run. Unlike a schedule trigger it is optional, and a present-but-blank value is rejected.

  A configured `jira` trigger is passive: it analyzes the matched issue and creates no reply obligation, so nothing obliges a reply on the ticket. The built-in `jira_mention` paths are the ones that owe a reply.
</details>
