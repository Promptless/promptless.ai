...
<Aside type="note">
  `docs_root_url` must be a public `http` or `https` URL; a bare hostname like `docs.acme.com` also works. Promptless fetches this URL for the doc agent and the broken-link crawler, so it rejects values that use another scheme, embed credentials, set a port other than 80 or 443, or point at a private, internal, loopback, or reserved address. Promptless validates the value whenever you save your configuration—in the dashboard or by pushing `promptless.yaml`—and stores nothing if it's invalid.
</Aside>
