...
The `browser` section gives the screenshot agent a provider-independent way to authenticate the browser requests it makes—attaching authentication headers and query parameters to requests for specific origins, and seeding cookies before navigation. It has two optional blocks, `request_rules` and `cookies`, each keyed by a short kebab-case slug (matching `^[a-z0-9][a-z0-9-]{0,63}$`). The Form editor has no Browser tab, so edit this section in YAML mode.
