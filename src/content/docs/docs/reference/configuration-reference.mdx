...
You can also ask Promptless to make these changes for you in Slack or Microsoft Teams. See [Ask Promptless to update your configuration](/docs/get-the-most-out/ask-promptless-to-update-config).
...
Triggers define events that automatically initiate documentation work. These are passive intake events that require explicit configuration. Each trigger has a `trigger_type` and a `match` list—the trigger activates when any clause in the list matches, and fields within a clause are ANDed together.
...
<details>
  ...
  **Match clause fields:**
  ...
  | `trigger_on`          | Required. Events that activate the trigger: `opened`, `first_approval`, `merge`    |
...
Escalation channels notify you of configuration and operational problems. This is completely different from notifications about suggestions being created, updated, closed, or merged. For example, a test account is missing the feature flags to screenshot a screen. Or a PR references a Jira issue that Promptless can't read—either the Jira integration isn't connected, or it's connected but the project isn't accessible. See [Escalation channel](/docs/tune/notifications#escalation-channel) for the full behavior.

| Field             | Description                                                                                      |
| ----------------- | ------------------------------------------------------------------------------------------------ |
| `slack_channel`   | Slack channel name for escalations (no `#` prefix)                                               |
| `msteams_channel` | Microsoft Teams channel conversation ID (e.g. `19:…@thread.tacv2`) for escalations               |
| `email`           | List of email addresses for escalations (escalation only; not used for suggestion notifications) |

Escalations reach the escalation channel(s) even when suggestion notifications are off or no notification channel is set. When a platform's escalation channel is unset, Promptless raises escalations through your suggestion-notification routing on that platform.
...
| Field                         | Description                                                                                                                                                                                                                                                              |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `auto_create_pr`              | Automatically create PRs for suggestions, or merge requests on GitLab                                                                                                                                                                                                    |
| `auto_merge`                  | Automatically merge documentation PRs. On GitLab the merge is immediate and squashes, and a merge request that can't merge right away stays open for you to merge manually ([full detail](/docs/connect/doc-locations/gitlab-projects#automatic-merge-request-creation)) |
