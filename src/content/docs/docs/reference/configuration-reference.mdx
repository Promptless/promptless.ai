...
- Inaccessible GitHub repository (a newly added `github`, `github_oss`, or `github_enterprise` collection must point at a repository the connected Promptless GitHub App can reach)

When you add a new `github`, `github_oss`, or `github_enterprise` doc collection, Promptless checks whether the connected GitHub App can reach the repository and rejects the change when it can confirm the repository is unreachable, so most access problems are reported at save time instead of surfacing later when Promptless tries to use the collection. This check applies whether or not you set `default_branch`. To resolve it, grant the Promptless GitHub App access to the repository — or, if the path is misspelled or the repository was renamed, correct it — then save again. If access to a repository is removed after its collection was already added, that is caught later, when Promptless next tries to use the collection, rather than at save time.
