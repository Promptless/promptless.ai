...
<details>
  ...
  Activates when a Jira issue matches a configured project, label, or status—on the transition into that match.
  ...
  **Match clause fields:**
  ...
  At least one of `projects`, `labels`, or `statuses` must be set in each clause. Fields within a clause are ANDed and clauses are ORed; all three match case-insensitively.
