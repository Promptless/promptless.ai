...
On the **Policies** tab, the default policy and each rule—see [Policies](#policies) for how a default overlay and its ordered rules combine—present three labeled groups, in UI order:

- **Notifications** announces a new doc suggestion.
- **Escalations** routes anything needing a person—a question about a change, or an integration Promptless can no longer reach.
- **Publishing** governs a doc update once it's ready.

**Notifications** and **Escalations** each contain a select labeled **Slack channel** and a select labeled **Microsoft Teams channel**—only the group heading above distinguishes them. The pair under **Notifications** routes suggestion announcements; the pair under **Escalations** routes items needing a person. Despite the plural group label, the **Notifications** selects write the singular `notification` key: **Slack channel** writes `notification.slack_channel`, and **Microsoft Teams channel** writes `notification.msteams_channel`.

On a rule, each of the four channel selects maps to the tri-state resolution described under [Policy fields](#policy-fields):

- **Inherit from default** omits the field.
- **Disabled — no Slack notifications**, **Disabled — no Teams notifications**, **Disabled — no Slack escalations**, and **Disabled — no Teams escalations** are the Form-mode equivalent of `null`.
- Picking a **channel name** routes to that channel.

The default policy has nothing to inherit, so its four selects offer **No Slack notifications**, **No Teams notifications**, **No Slack escalations**, and **No Teams escalations** instead. Selecting one of those four options is what stops announcements or escalations on that platform.

On a rule that routes nothing for a group, that group's two selects are hidden and an **Add notifications** or **Add escalations** button appears instead; clicking it reveals the two selects, both defaulting to **Inherit from default**. Clearing both channels stops routing for that group.

Under **Publishing**, the default policy shows a switch for each flag. On a rule, each flag is a three-way segmented control instead—**Inherit**, **On**, **Off**—so a rule can force a flag on or off against an inherited value. The switch and the segmented control write the same YAML keys:
...
You pick from a list rather than typing a channel name: the Slack selects list the channels the Slack integration can see; the Teams selects list the channels Promptless has seen activity in. When none are available, the **Microsoft Teams channel** select under **Notifications** falls back to pasting a channel's conversation ID. That paste fallback can inherit or route to a channel, but it can't disable an inherited Teams channel—switch to YAML mode and set `msteams_channel: null` for that.
