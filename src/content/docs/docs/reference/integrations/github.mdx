...
If your GitHub organization enforces an IP allow list, Promptless needs to reach your organization from its own network, or GitHub blocks its requests. When Promptless's traffic isn't permitted, GitHub returns a `403` on every authentication attempt, which silently stops Promptless from reading pull requests, opening documentation pull requests, and pushing updates.
