...
Connecting Microsoft Teams requires a Microsoft admin to approve the app for your tenant. If you're not an admin, start the connection during onboarding and hand the approval off to someone who is.

1. **Request access**: Start the connection during onboarding. Promptless records the request and shows a pending card with the time you made it, so the outstanding request stays visible to you and your teammates even after you reload the page.
2. **Ask an admin to approve**: A Microsoft admin approves the app for your tenant.
3. **Finish connecting**: After an admin approves, reopen setup and finish connecting Teams.

Promptless can't access your Teams tenant to track the admin's approval status, so the pending card doesn't update on its own. On the card, select **Check the status of your request** to reopen setup and continue once the app is approved. Select **Start over** to clear the pending request if you need to begin again.
