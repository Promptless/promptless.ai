...
Promptless also reads files attached in Teams **channels**, but only after a Microsoft tenant administrator grants it permission to read your organization's files. Until then, Promptless surfaces channel files as unavailable instead of reading them. Files attached in **group chats** aren't read yet—support for those is still in progress. Inline images work in every conversation regardless.
