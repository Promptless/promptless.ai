...
Some Slack workspaces require an admin to approve new apps before they can be installed. If you're not a workspace admin, Slack sends your installation to your admins as a request instead of connecting Promptless right away.

1. **Submit the request**: Complete the approval request in Slack. Promptless records it and shows a pending card during onboarding with the time you submitted it, so the outstanding request stays visible to you and your teammates even after you reload the page.
2. **Wait for approval**: Slackbot notifies you once an admin approves Promptless.
3. **Finish connecting**: After approval, return to the onboarding page and select **Complete connection** on the pending card.

Promptless can't see your Slack workspace to track the approval on its own, so the pending card doesn't update automatically. Select **Check the status of your request** on the card to open your workspace's Slack app-management page and confirm whether the request is still pending. To begin again, select **Start over** to clear the pending card in Promptless.
