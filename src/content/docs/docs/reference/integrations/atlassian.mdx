...
**How to attach.** An Atlassian organization admin first creates OAuth 2.0 credentials for a service account in Atlassian admin (Directory → Service accounts → create OAuth 2.0 credentials). From the Atlassian integration card, attach the service account by entering its **Client ID** and **Client secret** ("Attach service account"). The card then shows the attached account's display name, email, account ID, and client ID.
