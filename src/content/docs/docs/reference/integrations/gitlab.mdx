...
Promptless integrates with GitLab using group access token authentication, monitoring merge requests in your repositories for documentation updates, resolving GitLab.com MR links shared in triggers, and providing read access to source code for deeper context.
...
This allows Promptless to gather context from linked merge requests even when SAML/SSO login walls would otherwise block access.
