...
<Aside type="note">
  Some organization-wide configuration is admin-only in the dashboard. Only admins can change agent behavior settings or create, edit, and delete doc collections, though all members can view the current agent behavior settings and the list of doc collections. Editing the Agent Knowledge Base and the Configuration page (`promptless.yaml`) is also limited to admins, while everyone can view them.
</Aside>
