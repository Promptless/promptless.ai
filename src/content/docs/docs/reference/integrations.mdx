...
To route these notifications, set an `escalation` channel in your `promptless.yaml`. Until you do, these problems surface only on the Integrations page. Email escalation additionally requires email delivery to be configured for your deployment—it's enabled by default on Promptless-hosted deployments. See the [Configuration Reference](/docs/reference/configuration-reference) for the policies schema.
