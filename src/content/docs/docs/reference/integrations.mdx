...
If your organization has an [escalation channel configured](/docs/reference/configuration-reference#policies)—Slack, Microsoft Teams, and/or email—Promptless reports connection problems there, so you learn about a broken integration without checking this page.

Promptless posts about a given problem only once. It doesn't re-notify you day after day about the same ongoing issue.

To stop these notifications, open the [Integrations page](https://app.gopromptless.ai/integrations) and either disable or reconnect the affected integration. You can also simply ignore the alert—since Promptless notifies you only once per problem, doing nothing never triggers repeat alerts.
