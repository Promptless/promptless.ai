...
<Aside type="note">
  **When it's first approved** fires once per pull request, on the first approval only. Later approvals or re-approvals after requested changes don't trigger another run. If you merge your own pull requests without a separate reviewer, it never fires — use **When it's merged** or a commit trigger instead.
</Aside>
