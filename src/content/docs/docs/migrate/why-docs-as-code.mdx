...
Once your documentation lives in a GitHub repository, Promptless publishes to any of the Git-backed platforms it supports — Fern, Mintlify, ReadMe (Refactored), GitBook, Docusaurus, MkDocs, Hugo, Ghost, Nextra, Starlight, Vocs, and custom repo-based setups. See the [supported platforms list](/docs/connect/doc-locations/github-repos#supported-platforms) for the full set. The common thread is a Git-backed source, not a particular file format: Promptless works with Markdown and MDX, and also with DITA, reStructuredText, MadCap Flare, and FrameMaker (through a MIP intermediary). How you publish and which authoring format you use don't matter — Promptless just needs the source in a repository it can branch from.
