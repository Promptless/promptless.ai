...
**Minimal, high-signal context files.** If you maintain a CLAUDE.md, AGENTS.md, or [llms.txt](/blog/technical/agent-context-files-explained) for your product, keep it short. The ETH Zurich data suggests under 200 lines as a reasonable ceiling. Focus on what is non-obvious. Cover authentication quirks and rate limits that are lower than developers expect. Do not restate what the reference docs already say.
