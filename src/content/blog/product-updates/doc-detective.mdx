...
specifications describing your docs' procedures in JSON, YAML, or inline in your Markdown files, and Doc Detective executes them against the real
...
You can view the full source file in [our docs repo](https://github.com/Promptless/promptless.ai/blob/main/src/content/docs/docs/integrations/slack-integration.mdx).
...
time someone changes the docs or the product. Doc test failures appear in the
same pipeline results as your code test failures. The coupling between docs and
product becomes something you actively monitor, not something that silently
degrades.
...
Here are some concrete ideas for how to use Doc Detective. It can do a lot, so don't be overwhelmed by the list below.

### Use case 1: Your product has a UI, and your docs walk users through it

Your docs tell users to go to a page, click a button, fill in a field, and see a
...
or other matching strategies, clicks them, types into input fields, and
takes screenshots.
...
API documentation is full of example requests and expected responses, each one promising users what they'll get back. When someone changes the API
...
Python, JavaScript, and Bash commands from your docs and checks exit codes,
stdout matching (literal or regex), and stderr. When someone changes the CLI output
...
and reload session cookies. Others make it difficult. Clerk, for instance, issues auth cookies with a 60-second lifetime
that are refreshed by client-side JavaScript. This is how Clerk implements SSO
...
You have dozens or hundreds of screenshots in your docs. When a new release ships,
without tooling someone has to check each one manually. With Doc Detective, every screenshot is retaken during the test run
