...
* **Sidebar overlap on short screens:** On short viewports, the dashboard sidebar's sections could overlap, and the compact icon rail wouldn't scroll to reveal items below the fold. The sidebar now scrolls vertically when its contents don't fit, so every section stays reachable at any window height.
