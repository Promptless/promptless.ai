...
* **Publishing rules now apply to follow-on suggestions:** Fixed an issue where `policies.rules` keyed on `if.trigger` didn't apply to suggestions created during a follow-on—such as a backport requested by commenting on a documentation PR. These suggestions now inherit the matched trigger keys from their originating event, so trigger-scoped publishing settings like `auto_create_pr` resolve consistently across a suggestion's full lineage. Collection-scoped rules continue to apply only within their own collection.
