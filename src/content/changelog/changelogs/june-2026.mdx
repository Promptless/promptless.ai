...
* **Backfill suggestions returning zero pull requests:** Previewing or launching a backfill for a GitHub pull request or commit trigger could return no pull requests when your organization connected GitHub through more than one installation. This affected both triggers scoped with `repo_owners` and triggers listing explicit repositories that spanned multiple GitHub owners. Promptless now resolves each configured repository to the installation that owns it, so these backfills list the right pull requests.
