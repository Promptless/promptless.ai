...
* **Placeholder users in Slack mentions:** When owner routing resolved to a seeded or example Slack profile (such as "John Doe"), suggestion notifications could @mention that placeholder account by mistake. Promptless now skips placeholder and example profiles when resolving notification owners, so notifications only mention real teammates.
