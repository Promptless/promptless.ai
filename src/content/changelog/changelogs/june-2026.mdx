...
* **Doc agents use your existing skills:** If you already keep Agent Skills in your documentation repository, Promptless now uses them automatically—no extra setup required. Skills under `.claude/skills/`, `.agents/skills/`, or `.cursor/skills/` are picked up, and when a documentation task matches a skill's description, the agent runs that skill instead of deriving the workflow from scratch. You can guide when and how Promptless reaches for your skills by adding instructions to the `PROMPTLESS.md` file in your Knowledge Base. See [How Promptless Learns Your Docs](/docs/for-docs/connect/doc-locations/how-promptless-learns-your-docs#use-your-existing-skills) for details.

**Major Update:**

* **File-first `promptless.yaml` configuration:** All Promptless configuration now lives in a single `promptless.yaml` file in your organization's Agent Knowledge Base. This replaces the previous Projects page with a unified [Configuration page](/docs/for-docs/reference/configuration-reference). The file has four sections: `doc_collections` (where docs live), `triggers` (events that initiate work), `context_sources` (integrations for additional context), and `policies` (publishing and notification rules). Existing Projects configurations have been automatically migrated. Your triggers and doc collections continue working without any action required.
...
* **Structured configuration editor:** The [Configuration page](/docs/for-docs/reference/configuration-reference) now opens in a structured **Form** editor that groups your `promptless.yaml` into Doc collections, Triggers, Context sources, and Policies tabs, so you can manage everything without hand-editing YAML. Form mode saves one item at a time and commits each change to your Agent Knowledge Base right away, with pickers that autocomplete from your connected repos, Slack channels, and Jira, Confluence, Linear, and Notion inventory. The raw YAML editor stays available behind the **YAML** toggle.

* **Backfill suggestions for pull request triggers:** Pull request and merge request triggers (GitHub PRs, GitLab merge requests, and Bitbucket PRs) now have a **Backfill suggestions** panel that runs Promptless on pull requests that already merged. Preview the pull requests a trigger would have matched over the last 14, 30, or 90 days, then launch a backfill to process them live. See [Backfill Suggestions](/docs/for-docs/connect/triggers#backfill-suggestions).

* **Microsoft Teams notification channel:** Promptless can now announce documentation suggestions in a Microsoft Teams channel, not just Slack. Add `msteams_channel` to your notification policy with the channel's conversation ID (e.g. `19:…@thread.tacv2`) on the [Configuration page](https://app.gopromptless.ai/configuration). Slack and Teams channels are independent, so you can notify either or both. See [Customizing Notifications](/docs/for-docs/tune/notifications) for details.

* **Self-serve Microsoft Teams connect:** You can now connect Microsoft Teams to Promptless on your own—no setup call required. Download the Teams app package, install it into your Teams tenant, then click **Link App & Grant Read Channels** on the [Integrations page](https://app.gopromptless.ai/integrations) to link your tenant to your Promptless organization. See the [Microsoft Teams integration](/docs/for-docs/reference/integrations/microsoft-teams) setup guide for details.
...
* **Microsoft Teams channel access and channel picker:** When linking the Microsoft Teams integration, an admin can now grant Promptless read-only access to your teams and channels through Microsoft Graph by choosing **Link App & Grant Read Channels** on the integration card. Once granted, your synced channels appear in a picker when you select a Microsoft Teams notification channel in your configuration policies, so you no longer need to paste a raw conversation ID. Channel visibility is scoped to the authorizing admin's memberships: admin consent grants an org-wide permission, not org-wide visibility.

* **Read files and images you share in Microsoft Teams:** When you message @Promptless in a personal (1:1) Teams chat, Promptless now reads the files and images you attach and uses them as context for the docs it drafts, just as it does in Slack. Inline images you paste into a message are read in personal chats and in channels. Files shared in Teams channels are read too, but only from the SharePoint sites a Microsoft tenant administrator grants access to—after approving the connection, an admin picks those sites on the [integrations page](https://app.gopromptless.ai/integrations). Files on any other site are marked as unavailable. Promptless does not read anything you attach in a group chat, including inline images.

**Improvements:**

* **GitLab merge request link resolution:** When you share a GitLab.com merge request link in Slack or another trigger context, Promptless now fetches the MR metadata, discussion comments, and diff via the GitLab REST API. The feature uses your existing GitLab group access token and works automatically for any GitLab.com MR link.

* **Research breakdown shows Knowledge Base files and skills:** Each trigger's research breakdown—on the Triggers page and a suggestion's **Triggers & Analysis** tab—again summarizes what Promptless reviewed before drafting or skipping documentation, with cleaner repository-relative file paths. It now adds a **knowledge base files** category listing the [Knowledge Base](https://app.gopromptless.ai/settings/knowledge) files the agent read, such as contribution guidelines, and a "Used N of your skills" callout when the agent ran one of [your own skills](/docs/for-docs/connect/doc-locations/how-promptless-learns-your-docs#use-your-existing-skills). Each search also shows whether it ran against your doc collection or the agent's sandbox. See [Using the Web Interface](/docs/for-docs/work-the-queue/web-interface#research-breakdown).
...
* **Rebuilt onboarding as a connect-only flow:** The onboarding wizard is now five streamlined steps focused on connecting integrations: Connect Chat (Slack/Teams), Connect Docs (GitHub), Connect Triggers (GitHub/GitLab/Bitbucket), Connect Context (Atlassian/Notion/Linear/Slite), and Review Set-up (optional setup call). Only Connect Docs is required—the rest are skippable. Configuration now lives in your `promptless.yaml` file, which you can view and edit anytime on the [Configuration page](/docs/for-docs/reference/configuration-reference).

* **Streamlined Slack onboarding:** Connecting Slack during onboarding now completes in place instead of navigating to a separate page. The connected card displays your workspace name and icon.

* **GitLab source code reading:** Promptless can now read source files directly from your GitLab.com repositories when processing merge requests. This allows Promptless to gather context from implementation details beyond what's visible in MR diffs, resulting in more accurate documentation suggestions. See [GitLab Integration](/docs/for-docs/reference/integrations/gitlab#source-code-reading) for details.
...
* **Integration connection health on the Integrations page:** Promptless now checks each connected integration daily to confirm its credentials are valid and the third-party service is reachable. The [Integrations page](https://app.gopromptless.ai/integrations) shows a health label on every connected integration—**Connected**, **Needs reconnect**, or **Temporary outage**—with an info tooltip showing when Promptless last confirmed the integration healthy and, during an ongoing outage, when it first and last detected the issue. **Needs reconnect** means the integration's authorization expired and you need to reconnect it; **Temporary outage** clears on its own once the service is reachable again.
...
* **Microsoft Teams mentions not triggering responses:** Fixed an issue where @Promptless mentions in Microsoft Teams channels and DMs were silently ignored. The Teams integration now correctly processes all incoming messages.
...
* **Refreshing a Microsoft Teams integration:** A refresh now re-resolves your tenant's organization name and rebuilds the channel list for every team Promptless is installed in, including channels Promptless hasn't been mentioned in yet. Existing installations need to re-upload the latest Promptless app package in the Teams admin center to grant the new channel-read permission this relies on.
