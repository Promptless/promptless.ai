...
* **Knowledge base usage analytics:** A new [Knowledge base page](https://app.gopromptless.ai/knowledge) shows which Agent Knowledge Base files Promptless agents actually read while working on your documentation, and how often each one is consulted. A daily chart breaks reads down by file and marks how many agent runs consulted the knowledge base, and a per-file table reports sessions, total reads, distinct versions read, and last read—each row links straight to that file in the Agent Knowledge Base editor. Use it to confirm that the guidance you've written is steering the agent.
