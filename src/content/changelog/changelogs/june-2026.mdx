...
* **File-first `promptless.yaml` configuration:** All Promptless configuration now lives in a single `promptless.yaml` file in your organization's Long-Term Context repository. This replaces the previous Projects page with a unified [Configuration page](/docs/configuring-promptless/configuration-reference). Your configuration is now fully git-versioned and inspectable in one place. The file has four sections: `doc_collections` (where docs live), `triggers` (events that initiate work), `context_sources` (read-only integrations for additional context), and `policies` (publishing and notification rules). Existing Projects configurations have been automatically migrated—your triggers and doc collections continue working without any action required.

* **Structured configuration editor:** The [Configuration page](/docs/configuring-promptless/configuration-reference) now features a form-based editor as the primary interface. Manage doc collections, context sources, triggers, and policies through guided form controls without editing YAML directly. The editor shows a diff preview before committing, so you see exactly what changes will be saved. The raw YAML editor remains available via a toggle for advanced users who prefer direct file editing.
...
* **Configuration page accessible to all members:** All organization members can now view the Configuration page to see how Promptless is set up. Only admins can edit and save changes—non-admins see a read-only editor with "View only — admins can edit" instead of the Save button.
