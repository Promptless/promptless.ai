...
* **GitLab merge request badges on Triggers & Analysis:** GitLab MR trigger cards on the Triggers & Analysis tab now show a dedicated GitLab badge with a colored lifecycle glyph for opened, merged, closed, or draft, next to the project path and MR number. GitLab MRs previously fell back to a generic label; the new badge matches the GitHub PR badges so you can tell GitLab triggers apart at a glance.
