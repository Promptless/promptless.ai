...
* **@promptless mentions from bots on documentation PRs:** Follow-on comments on a documentation PR dropped comments from automated accounts before checking for an @promptless mention, so a CI or review GitHub Action that tagged Promptless on a docs PR couldn't request a follow-on revision. A direct @promptless mention now takes priority over the bot filter on docs PRs too, matching the existing behavior on source PRs. Bot comments without a mention are still ignored.
