...
* **Missing Slack notifications for channels addressed by name:** In very large Slack workspaces, a temporary Slack rate limit while Promptless listed channels could leave the channel inventory empty, silently stopping notifications from reaching channels addressed by name rather than by ID. Promptless now follows Slack's rate-limit guidance and retries the channel listing with backoff, so the inventory fills correctly and name-based delivery stays reliable.
