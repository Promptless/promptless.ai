...
* **Choose an organization when enrolling a host:** The host enrollment screen now shows a searchable list of the organizations you belong to, and you pick which one to enroll the host into before approving it. Enrollment uses the organization you select instead of the browser's active organization, which could fail to find the host's deployment when the two didn't match. See [Enroll a host](/docs/governance/get-started/enroll-your-hosts#enroll-a-host).
