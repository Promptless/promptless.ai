...
* **See when Promptless last received a GitLab webhook:** Each saved GitLab merge request trigger now shows a webhook delivery status box on the Configuration page. It shows, in your time zone, when Promptless last received a merge request webhook from the trigger's watched projects. Deliveries from before the box appeared count too. The box color shows how recently that webhook arrived, so you can confirm at a glance that your GitLab events are reaching Promptless. A new GitLab connection gets 4 days to receive its first webhook before a trigger shows the "No webhook recorded yet" warning. Existing connections need no reconnection. See [GitLab merge requests](/docs/for-docs/connect/triggers/gitlab-merge-requests#webhook-delivery-status).
