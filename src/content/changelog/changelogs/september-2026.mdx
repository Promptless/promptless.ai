...
* **Register self-hosted analyzers from the Dashboard with a single credential:** Add an analyzer in **PIG → Settings** by giving it a name. Promptless creates the installation, shows it as **Awaiting connection**, and reveals one credential to copy into your secret manager. The analyzer resolves its installation identity from that credential and connects to Promptless's hosted endpoint (`https://api.gopromptless.ai`) by default. Rotate or revoke the credential per installation from the same page; the installation keeps its history and enrolled hosts, and rotation takes effect immediately. See [Register an analyzer](/docs/governance/deploy-the-worker/plan-your-deployment#register-an-analyzer).
