...
* **Answer an MCP clarifying question without waiting for the task to finish:** When a Promptless task asks a clarifying question over MCP—in Claude Code, Cursor, or another editor—you can now answer it while the task is still working, not only after it finishes. Your answer goes to that same task, which picks up where it paused and keeps going; a question from a task that has already finished still continues the work in a new task, as before. It's available to every organization. See [MCP triggers](/docs/for-docs/connect/triggers/mcp#answer-a-clarifying-question).
