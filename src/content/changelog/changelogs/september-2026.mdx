...
* **Get one suggestion from your recent merges with a New Task template:** The New Task composer's **Use a template** rail now starts with **Find doc update from recent merges**. Select it and the instructions field fills with a complete, ready-to-submit prompt: review the newest merged pull requests or merge requests in your connected source repositories, find one change that needs a documentation update, check the current docs and existing suggestions to avoid duplicate work, and create one focused suggestion. It runs in Default mode, so there are no placeholders to fill in and no Deep Analysis run to wait on. When your organization has no open suggestions, such as right after setup or once you've closed the last one, the card shows a **Recommended** badge so you know where to start. You can also open it directly from `app.gopromptless.ai/new-task?template=recently-merged-changes`. See [Find a doc update from recent merges](/docs/for-docs/work-the-queue/web-interface#find-a-doc-update-from-recent-merges).
