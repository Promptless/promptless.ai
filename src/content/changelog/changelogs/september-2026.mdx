...
* **Find doc update from recent merges:** You can now select this template from the top of the New Task composer's **Use a template** rail to turn your latest merges into a documentation suggestion. It runs a **Default**-mode task that reviews your most recently merged pull requests and merge requests, checks your current docs and open suggestions to avoid duplicating work, and creates one focused suggestion for a change that needs documenting. When your organization has no open suggestions, the card shows a **Recommended** badge marking it as a good place to start. See [Use a template](/docs/for-docs/work-the-queue/web-interface#use-a-template).
