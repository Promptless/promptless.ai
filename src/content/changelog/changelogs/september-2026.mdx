...
* **Grade suggestions by urgency and importance:** Promptless now grades every suggestion it creates for urgency and importance, adding `urgent`/`not urgent` and `important`/`not important` labels so you can see at a glance which suggestions need attention first. Write your own rubric in `doc_workflow/docs_impact_grading.md` in your Agent Knowledge Base to move where Promptless sets the bar; its default rubric applies until you do. See [Filter noise and tune relevance](/docs/for-docs/tune/noise-and-relevance-filtering#how-promptless-grades-suggestions-for-urgency-and-importance).
