...
* **Select all matching works while the suggestions list is still loading:** On the suggestions list, **Select all matching** stays available while the list is still loading, so you can run bulk actions on a large backlog as it loads. Starting a bulk action before the list finishes covers the suggestions loaded so far. The confirmation dialog notes when the list is still loading, so you can run it again once the rest are in. See [the suggestions list](/docs/for-docs/work-the-queue/web-interface#bulk-opening-pull-requests).
