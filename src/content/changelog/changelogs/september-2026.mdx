...
* **Promptless stays quiet when a docs PR comment is addressed to someone else:** Whenever Promptless processes a comment on a documentation PR, in either comment trigger mode, it now checks who the comment addresses. Promptless stays quiet—no reply and no changes, even on a closed or merged suggestion—only when a comment assigns its question or request exclusively to another named participant and asks nothing of Promptless. A comment that doesn't clearly single out someone else, or that still asks something of Promptless even while naming another participant, still gets a reply.
