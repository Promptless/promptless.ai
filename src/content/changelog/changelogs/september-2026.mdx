...
* **Reach your Agent Knowledge Base from your own tooling:** Read and write your organization's Agent Knowledge Base with your Promptless API key. Clone it over Git to read the whole corpus with its history. Create or replace one file at a time over the API. Your own tools and automation now work from the same personas, style rules, and product context Promptless reads on every task. See [Agent Knowledge Base access](/docs/for-docs/connect/triggers/api#agent-knowledge-base-access).
