...
subtitle: Follow your tasks to the finish from your editor, see backfill status and suggestions as any organization member, and read richer suggestion comments in the dashboard that now render Markdown, support teammate @-mentions, and stay visible after their referenced text changes.
...
* **Review the docs updates Promptless plans before it drafts them:** When Promptless decides not to draft a change automatically, it now records the plan as a doc update finding on the new Doc Plans page. You can edit a finding, draft it into a suggestion, or dismiss it. A new [Autonomy setting](/docs/for-docs/work-the-queue/doc-plans#configure-how-much-promptless-drafts) controls how much Promptless drafts on its own, from every warranted change down to none, and you can list or draft findings over [MCP](/docs/for-docs/connect/triggers/mcp#available-tools), and list, draft, or dismiss them over the [API](/docs/for-docs/connect/triggers/api#doc-update-findings). See [Doc Plans](/docs/for-docs/work-the-queue/doc-plans).
