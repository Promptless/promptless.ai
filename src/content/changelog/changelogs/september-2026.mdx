...
* **Grant missing Slack permissions on your existing connection:** Promptless now checks that your connected Slack workspace can list private channels (`groups:read`) as well as render link previews. When a permission is missing, the Slack card shows a **Permissions needed** label—separate from the **Needs reconnect** label that flags a lapsed authorization—with a callout naming what to add and a **Reconnect Slack** button. Selecting it re-runs Slack authorization to add the permission, and your connection, channels, and configuration stay intact.
