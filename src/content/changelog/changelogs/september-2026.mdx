...
* **Poll for finished documentation tasks and branch on their outcome:** A new `GET /v1/triggers` endpoint lists your finished tasks, newest completion first. An API or Zapier automation can act the moment a task ends, including a task that deliberately produced no documentation change. Each task carries an `outcome` field—`suggestions_created`, `no_change_needed`, `needs_input`, or `failed`—that an automation can branch on, alongside the `resolution` prose that explains it. The same `outcome` field now appears on `GET /v1/triggers/{trigger_event_id}` and MCP `get_task_status`. In Zapier, the **Documentation Task Finished** trigger polls this feed. See [API triggers](/docs/for-docs/connect/triggers/api#finished-tasks).
