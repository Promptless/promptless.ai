...
* **See when Promptless last received a GitLab webhook:** Each saved GitLab merge request trigger now shows a webhook delivery status box on the Configuration page. It reports the most recent merge request webhook Promptless received across the trigger's watched projects, in your time zone, and colors the box by how recently that webhook arrived. You can confirm at a glance that Promptless is receiving your GitLab events. A trigger with no recorded delivery reads "No webhook recorded yet," and existing connections show their earlier webhook history with no reconnection. See [GitLab merge requests](/docs/for-docs/connect/triggers/gitlab-merge-requests#webhook-delivery-status).
