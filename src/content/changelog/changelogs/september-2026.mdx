...
* **A New Task template that turns recent merges into one suggestion:** The **Use a template** rail in the New Task composer leads with **Find doc update from recent merges**. It runs in **Default** mode, and its prompt is ready to submit as written—no placeholders to fill in. Promptless works through the recently merged pull requests or merge requests in the source repositories connected to your docs, newest first, finds one change that needs a documentation update, and creates a single focused suggestion for it, checking your current docs and existing suggestions first so it doesn't duplicate work. When your organization has no open suggestions, the card carries a **Recommended** badge—a pointer to a good next task, not a selection. See [New Task templates](/docs/for-docs/work-the-queue/web-interface#new-task-templates).
