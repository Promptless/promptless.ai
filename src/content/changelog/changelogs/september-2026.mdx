...
* **Follow a New Task as a conversation in the dashboard:** Submitting a New Task opens its own conversation page. Promptless acknowledges the request, posts progress, answers you or asks a clarifying question, and reports the outcome in the thread, and you can reply while the task runs. Reply to a finished task to start a follow-up that continues the conversation. A Recent tasks list in the sidebar shows your organization's task conversations from the last 90 days, and tasks you submit over MCP or the API appear there too. For a New Task, the **Suggestions and blockers go to** control chooses a Slack channel for that task's suggestion notifications and blockers, or turns them off. See [the web interface](/docs/for-docs/work-the-queue/web-interface#task-conversations).
