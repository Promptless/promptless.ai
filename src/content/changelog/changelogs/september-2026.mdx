...
* **Enroll hosts that have no browser:** You can now enroll a PIG host you reach over SSH, or any other host without a local browser. Run the host runtime's `enroll` command with `--device`, then open the printed approval link on another device and approve the host there. See [Enroll your hosts](/docs/governance/get-started/enroll-your-hosts).
