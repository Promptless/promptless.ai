...
* **Discussions can't be resolved while Promptless is still working on them:** In the dashboard, a suggestion-review discussion can't be resolved while Promptless has queued or in-progress work on it. During that window the **Resolve** control is disabled and explains on hover that you should wait for Promptless to finish working on the discussion before resolving it. An indicator showing that Promptless is working still appears, so review feedback stays visible while Promptless keeps processing it. Reopening a resolved discussion still works, and so does resolving a discussion once its work finishes.
