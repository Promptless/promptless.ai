...
* **Notifications link back to their source and named tickets:** Suggestion notifications now include a link to the merge request or ticket that started the work, when the trigger carries one, and a ticket, issue, or merge-request key your notification names, such as `INC-879`, renders as a link to its tracker when Promptless can resolve it. State your own key-prefix URL patterns in `notification_preferences.md`. See [Notification preferences file](/docs/for-docs/tune/notifications#notification-preferences-file).

* **Fixed labeled-line format for notifications and escalations:** Suggestion notifications and escalations now follow a fixed labeled-line format, and every escalation opens with a banner line that lets you tell a blocker needing your input apart from an informational pointer at a glance. See [Notification preferences file](/docs/for-docs/tune/notifications#notification-preferences-file) and [Escalation channel](/docs/for-docs/tune/notifications#escalation-channel).
