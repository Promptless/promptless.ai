...
* **Create a Jira issue on request:** Ask Promptless in Slack or Microsoft Teams to create a Jira issue in a target project. Promptless checks the project for a matching issue first and reuses it instead of creating a duplicate. The created issue is reported by the connecting account that authorized the Atlassian connection. See [Create a Jira issue on request](/docs/for-docs/reference/integrations/atlassian#create-a-jira-issue-on-request).
