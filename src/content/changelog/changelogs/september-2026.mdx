...
subtitle: Follow your tasks to the finish from your editor, see backfill status and suggestions as any organization member, read richer suggestion comments in the dashboard that now render Markdown, support teammate @-mentions, and stay visible after their referenced text changes, and get confirmation on a Jira issue as soon as Promptless picks up a request.
...
* **Review documentation once per release with a deployment trigger:** The new deployment trigger type runs a documentation review once per release, when you push a release tag matching a pattern you choose. It's a good fit for high-merge-volume organizations that ship a few times a day. See [GitHub deployments](/docs/for-docs/connect/triggers/deployment).
