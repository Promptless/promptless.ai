...
* **Automatic setup when Promptless opens a doc collection:** Add an `on-mount.sh` script to a doc collection's folder in the Agent Knowledge Base. Promptless runs it each time it opens that collection, so it can install dependencies or start a local docs server before it works. The script runs with your configured environment and tolerates repeated runs, and Promptless stops its servers when the task finishes. See [Collection setup scripts](/docs/for-docs/get-the-most-out/agent-knowledge-base#collection-setup-scripts).
