...
* **A recommended first task when your queue is empty:** The New Task composer now leads its **Use a template** rail with **Find doc update from recent merges**, a ready-to-submit template that reviews your most recently merged pull requests and merge requests, checks your current docs and open suggestions to avoid duplicating work, and creates one focused suggestion for a change that needs documenting. When your organization has no open suggestions—on a new account or after you've cleared the queue—the card is marked **Recommended** so there's an obvious next step. It runs in Default mode and produces a single suggestion. See [Use the web interface](/docs/for-docs/work-the-queue/web-interface#recommended-first-task).
