...
* **Grade suggestions by priority:** Promptless now grades every suggestion it creates with one priority level—`critical`, `high`, `medium`, or `low`—and marks it with a `priority:<level>` label so you can see at a glance which suggestions need attention first, then filter your backlog to a level from the suggestion's priority badge. Write your own rubric in `doc_workflow/docs_impact_grading.md` in your Agent Knowledge Base to move where Promptless sets the bar; its default rubric applies until you do. See [Filter noise and tune relevance](/docs/for-docs/tune/noise-and-relevance-filtering#how-promptless-grades-suggestions-by-priority).
