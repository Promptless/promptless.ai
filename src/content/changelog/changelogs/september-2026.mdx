...
subtitle: Suggestion comments in the dashboard now render Markdown, and comments and citations stay visible after their referenced text changes.
...
* **Filter suggestions by GitLab MR source repository:** The trigger-source filter on the suggestions list now includes a GitLab MRs group alongside the existing GitHub group, so suggestions can be filtered by the GitLab merge request source repository they came from. This brings GitLab to parity with the existing GitHub behavior. GitLab MR-triggered suggestions already appeared in the list. Now the list can be narrowed to the ones from a specific source repository. The GitLab MRs group works the same way as the GitHub group: whole-group or individual repository selection, with independent collapse and expand. Mixed GitHub and GitLab selections persist through saved, shared, and bookmarked filter URLs. It's available now, with no new setting or trigger to configure.
