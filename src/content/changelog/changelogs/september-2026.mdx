...
* **Choose an organization when enrolling a host:** When you belong to more than one organization, the host enrollment screen shows a searchable list to pick from before you approve the host. If you belong to only one, Promptless selects it and starts enrollment automatically. Enrollment uses the organization you select instead of the browser's active organization, which could fail to find the host's deployment when the two didn't match. See [Enroll a host](/docs/governance/get-started/enroll-your-hosts#enroll-a-host).
