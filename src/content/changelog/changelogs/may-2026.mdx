...
* **Doc Collection Path Scope Enforcement:** Fixed path scope settings (documentation directories) not being enforced at runtime. Path scopes configured in the UI are now enforced when agents create suggestions—changes to files outside the configured scope are rejected with a clear error message. If you configured path scopes expecting enforcement, they now work as intended.
