...
* **Start a Promptless session from a Jira ticket:** A Jira ticket can now start a Promptless session—assign the ticket to Promptless, add a `promptless` label, @mention Promptless in a comment, or configure a trigger on a Jira project, label, or status—and Promptless replies on the ticket with the documentation PR linked. See [Jira triggers](/docs/connect/triggers/jira).
