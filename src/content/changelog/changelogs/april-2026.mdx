---
title: April 2026
subtitle: Multi-org GitHub connect, first-approval PR triggers, and improved Slack notifications.
...
* **Suggestion Editor Validation:** The suggestion editor now validates documentation files in real time as you edit. Inline markers highlight syntax issues in Markdown, MDX, reStructuredText, and AsciiDoc files—including broken frontmatter, merge conflict markers, and parser errors. A warning banner summarizes detected issues, and a confirmation dialog ensures you acknowledge any warnings before saving.
...
* **Bulk Open Pull Requests:** Open pull requests for multiple suggestions at once from the suggestions list. Click Select to enter select mode, check the suggestions you want, then click Open N PRs to create PRs for all eligible suggestions in a single action—useful for clearing a backlog of reviewed suggestions.
* **Slack Thread Auto-Listening:** Promptless now automatically processes replies in Slack threads where it's participating—threads where it posted or was @mentioned. Reply with feedback, images, or clarifications without needing to @mention Promptless again. Start a message with `aside` to skip triggering. Configure in Organization Settings; default is auto-listening enabled.
* **Notion Context Source:** Connect your Notion workspace to give Promptless read-only access to your team's knowledge base. Select specific pages and databases as context sources, and agents can search Notion content, fetch page markdown, and query database rows when creating documentation suggestions.
* **LaunchDarkly Integration (Beta):** Connect your LaunchDarkly account to monitor feature flag changes. Create a service token with the Writer role (or a custom policy granting `webhook/*` permissions) and paste it in the integrations page. Promptless automatically creates a webhook in your LaunchDarkly account to receive flag change events. Full trigger support for automated documentation updates is coming in a future release.
* **Jira Issue Creation:** Create Jira issues directly from Promptless to track documentation work or link suggestions to existing workflows. Specify the project key, issue type, summary, and description—plus optional labels and links to related tickets. Works with both Jira Cloud and Jira Data Center.
...
* **Direct Commits for Docs PR Feedback:** When you @mention Promptless on a pull request that lives in your docs repository, changes now commit directly to that PR's branch instead of creating a separate suggestion branch—keeping all edits in your original PR without manual reconciliation.
* **GitHub OSS Public Repositories Only:** The GitHub OSS integration now explicitly requires public repositories. Private and internal repositories are filtered from the repo picker, and the backend validates visibility at doc collection creation time. Use the standard GitHub integration for private repositories.
* **Merged PR Visual Indicator:** The "Created PR" button on suggestions now turns purple and shows "Merged PR" after the docs PR is merged—mirroring GitHub's color convention so you can see shipped state at a glance without clicking through.
* **Markdown Preview for Suggestion Diffs:** Preview rendered Markdown directly from the suggestion diff view. Click "Preview Markdown" on any `.md`, `.mdx`, or `.markdown` file to open a modal showing the rendered content with GitHub-style diff highlighting—added lines appear with a green rail, removed lines with a red rail. The preview scrolls to match your position in the code editor.
...
* **Source PR Comment Delivery:** Fixed source PR comments failing to post for some GitHub App installations—a regression from April 12 caused comment delivery to silently fail for certain permission configurations.
* **Docs PR Branch Sync:** Fixed suggestion branches with open docs PRs being skipped during automatic base branch sync—docs PRs now stay synchronized with the main branch throughout their lifecycle.
* **Conflict Marker False Positives:** Fixed suggestion branches being blocked for repositories whose documentation legitimately contains git conflict marker examples—such as tutorials explaining how to resolve merge conflicts.
* **Slack Diff File Rendering:** Fixed an issue where diff file uploads in Slack would sometimes render as binary content instead of readable text—diffs now display correctly as syntax-highlighted code snippets.
* **Source Trigger Links in Docs PRs:** Fixed docs PRs not including links to the source triggers (feature PRs, Slack threads, etc.) that initiated the documentation work—a regression since March 2026. Docs PR descriptions now include a "Triggered by" section linking back to the original context.
* **Slack User Mention Display:** Fixed Slack user mentions appearing as raw tokens (like `<@U080S8KGDUP>`) in the dashboard, suggestion details, and generated GitHub PR descriptions—mentions now display as readable `@display_name` text.
* **Duplicate Manual Merge Notices:** Fixed duplicate "merge manually" Slack notices appearing when clicking Publish multiple times for suggestions where auto-merge is blocked by policy—such as OSS repositories or protected branches.
* **GitHub Pasted Image Processing:** Fixed images pasted directly into GitHub issues or PRs not being processed when they trigger documentation work—Promptless now properly downloads these user-attachment images for use in documentation.
* **Citation Review Comments on Auto-Published PRs:** Fixed citation review comments not appearing on docs PRs created via auto-publish pipelines—a regression since early April that caused inline citations to display only in the Promptless suggestion UI rather than as review comments on the PR diff.
* **GitHub PR/Issue Reply Channel:** Fixed clarification and escalation replies going to Slack when a user @mentioned Promptless on a GitHub PR or issue—replies now appear on the same PR or issue where the request was made.
* **Docs PR Status After Repository Rename:** Fixed docs PR status (open/merged/closed) not updating correctly when the docs repository is renamed—webhook events would arrive with the new repo URL but fail to match suggestions stored with the old URL. PR status now syncs reliably through repo renames.
