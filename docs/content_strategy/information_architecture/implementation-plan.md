...
scope: Execution plan to cut over the Docs tab (src/content/docs/docs/for-docs/) from today's structure to the proposed IA, in incrementally reviewable phases
covers_nav_tab: docs
...
(`src/content/docs/docs/for-docs/`, served at `/docs/for-docs`) to the [proposed IA](proposed-ia.md),
...
1. **Move/rename the content file** under `src/content/docs/docs/for-docs/` to its new slug path (and fix
...
`src/content/docs/docs/for-docs/`. The tables below group every existing page by the **phase** that moves
...
| Current page (`/docs/for-docs/…`) | Target section | Notes |
|---|---|---|
...
- **Rule:** any page whose slug changes gets a redirect from its old `/docs/for-docs/...` path to its new
...
- Every current page in §2.1 resolves at its new slug, and every old path returns a redirect (no
  `/docs/for-docs/...` 404s) — verified by the broken-link check against a full crawl.
...
- Remove the old section landing pages and grouping that no longer have a home
  ...
  index.
- Delete emptied directories under `src/content/docs/docs/for-docs/` after their pages have moved.
