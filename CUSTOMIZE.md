...
- **API reference (`starlight-openapi`)** — `schema: ./public/openapi/api-triggers.yaml`, rendered under `/docs/for-docs/api/*` via the `starlightOpenAPI([...])` plugin, with `openAPISidebarGroups` wired into the Promptless for Docs topic in `astro.config.mjs`. Legacy `/api/*` routes redirect to this namespace.
- **Navigation** — directory-driven since Phase 3 (ADR 0003): `starlight-sidebar-topics` wraps Starlight's native folder `autogenerate` in `astro.config.mjs`. [ADR 0002](adrs/0002-split-documentation-by-product.md) supersedes only the single-topic decision in Starport ADR 0004 §2: there are now separate topics for Promptless for Docs and Promptless for Agent Instructions, selected through the shared product switcher. Placement/visibility come from each page's `sidebar.order`/`sidebar.hidden`; section/group labels are set explicitly in the topic config. Shared product labels, destinations, descriptions, icons, and badges live in `src/lib/docs-products.ts`. The old `buildSidebar()` → `src/lib/generated/sidebar.json` pipeline remains retired; `scripts/generate-manifest.ts` produces only `route-manifest.json` for the `.md` endpoints.
