...
- **Navigation** — driven today by `scripts/generate-manifest.ts` → `src/lib/generated/sidebar.json` (frontmatter-generated; do **not** hand-edit it). Phase 3 retires the `buildSidebar()` half of that pipeline for the template's folder-autogenerated `starlight-sidebar-topics`, reproducing the current labels/order in config. `route-manifest.json` generation stays (the `.md` endpoints need it).
