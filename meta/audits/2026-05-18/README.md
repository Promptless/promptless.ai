...
**Start here for team review:** [**report.html**](report.html) — single self-contained HTML file (~290 KB) with the full audit narrative, dashboard, filterable findings, sortable lens scoreboard, per-page data explorer, embedded Tier 2 reports, recommendations sequencing, and reusable artifacts. Opens in any modern browser; no external dependencies. Generated by [`evidence/build-report.mjs`](evidence/build-report.mjs) (idempotent — re-run after any change to update).

## How to read this audit (source markdown view)
